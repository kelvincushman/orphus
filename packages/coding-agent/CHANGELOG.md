...
- An interactive `browser` tool in `@orphus/web-access` drives a real Chrome through the DevTools Protocol — `open`, `read` (`text`/`dom`/`accessibility`/`screenshot`), `click`, `type`, `wait_for`, `close`, and `login` — for pages a plain fetch can't reach because they sit behind a form. `click` escalates automatically from a cheap synthetic click to a trusted `Input`-dispatched click only when the page detects and silently no-ops the synthetic one; there is no third rung, and no CAPTCHA-solving or anti-bot capability is shipped. Each session gets its own disposable, isolated Chrome instance, capped in count and killed on session shutdown. A new `/credential` command (human-typed only; the agent has no path to it) manages a site-credential vault backed by the OS keychain (macOS `security`, Linux `secret-tool`): `add <domain> <label> <username>` (secret supplied via the `ORPHUS_VAULT_SECRET` env var, never a command argument), `list`, `remove <domain> <label>`, and `confirm <domain>`. The `login` action fills a form from a stored credential without the password ever reaching the model — three gates enforced in order (the login flag, the domain allowlist, a first-use human confirmation) — and reports only the username back. A bundled `browser-operation` skill documents the sense→act→verify loop and the escalation ladder. Everything is behind `ORPHUS_ENABLE_BROWSER` and `ORPHUS_ENABLE_BROWSER_LOGIN`, both default off.

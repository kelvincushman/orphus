...
- `ProcessCapability.exec` resolves on stream close rather than process exit, so output written while the pipes drain — a grandchild holding stdout, most visibly — is no longer lost.

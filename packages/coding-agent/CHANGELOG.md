...
- **A second terminal renderer, opt-in, for startup selection and the session picker.** `ORPHUS_TUI_BACKEND=termdom` or `tui.backend: "termdom"` runs both surfaces — the labelled startup list and the `--resume` picker — on [termDOM](https://github.com/bikeshaving/termdom), which lays out real HTML and CSS in a terminal; the default stays `pi` and pi remains the immediate fallback. The picker is responsive — search, list, and a detail panel at 90 columns or wider, a single column below that — and supports mouse row selection alongside full keyboard parity. Parity is structural rather than maintained by hand: both renderers drive one `SessionSelectorModel` holding the filtering, threaded/recent/relevance sorting, named filter, scope toggle, regex and quoted search, tree construction, path visibility, rename, delete confirmation, loading state, and the safeguard that refuses to delete the session you are in; and termDOM's DOM key events are re-encoded into the raw key data the ordinary keybinding configuration matches, so a rebound key rebinds both. A backend owns terminal attachment, input, rendering, resize, and an **awaited** teardown, so raw mode, cursor, mouse reporting, and bracketed paste are restored before anything else attaches. The main chat shell is unchanged and always pi; every public `Component`, custom editor, renderer, and extension API is untouched.
- **Browser operation, behind `ORPHUS_ENABLE_BROWSER`.** A single action-dispatched `browser` tool — `open`, `navigate`, `snapshot`, `status`, `screenshot`, `click`, `type`, `login`, `close` — whose actions all share one browser session. That browser is never yours: each session launches its own Chrome with a throwaway `--user-data-dir` and tears it down afterwards, so it carries none of your cookies or logged-in sessions. With the switch off the tool is not registered at all, so it costs nothing in the tool list, the system prompt, or the context window. Observations (`snapshot`, `screenshot`, `status`) get a bounded retry because asking a page a question twice changes nothing; page-changing actions are never silently retried and report their uncertainty instead, pointing at `snapshot` to establish what actually happened. On close, cancellation, and shutdown every registered resource has its teardown attempted and awaited and the registry is emptied, and the result is reported as what the pass did — including failures — rather than as a zero-orphan guarantee.
- **Browser login, behind a second switch.** `ORPHUS_ENABLE_BROWSER_LOGIN` plus an origin allowlist in `ORPHUS_BROWSER_LOGIN_ORIGINS`. A secret reaches a page only when both switches are on, the page's origin is allowlisted, the credential is itself scoped to that origin, and a human approved that credential for that origin — asked once per session, and **denied outright when there is nobody to ask**, so an unattended run cannot authorize its own credential use. Secrets live in the OS keychain (`security` on macOS, `secret-tool` on Linux; Windows fails closed until a native binding exists) and are read only at the moment they are handed to the browser; the non-secret registry of labels, origins, and usernames lives in `<agent dir>/browser-credentials.json`. Only the label ever reaches a tool result, a log, or the approval prompt.
- **The harness records what it actually sent.** Every provider request is written to the session as an `orphus.provider.request.v1` custom entry at the last point before dispatch — after every `before_provider_request` hook and after payload sanitization — carrying the request id, retry attempt, turn, branch leaf, provider/model/API, and the exact body with its SHA-256 and byte length. Bodies up to 1 MiB stay inline; larger ones spill to `<session dir>/provider-requests/<id>.json` at mode `0600` in a `0700` directory, referenced by session-relative path. A request whose record cannot be written **fails before it reaches the network**, because a dispatch nothing recorded is a dispatch nothing can replay. Each request is closed out by an `orphus.provider.response.v1` entry with status, finish reason, usage, duration, and a normalized error. Authentication headers and credentials are never recorded; credential-shaped payload fields are redacted and listed, while the hash always describes the exact body sent. All of these are `custom` entries, so none of them enters reconstructed model context. Recording is on by default — `ORPHUS_PROVIDER_AUDIT=0` turns it off.
- **`orphus inspect runtime --json`** assembles the real runtime, dispatches nothing, and prints a deterministic versioned report: resolved provider/model, the implementation wired for each capability, every tool with the hash of its parameter schema, extensions with their handler counts and the order their hooks run in, flag and settings provenance, and a hash per top-level system-prompt section. Everything is sorted, so two runs of the same configuration are byte-identical and a diff is a real difference. `--include-content` adds the resolved prompt body and nothing else; secrets are redacted in both modes.
- **An injectable capability boundary.** Clock, filesystem, processes, credentials, browser, transcription, and terminal transport are now small interfaces passed to the session rather than module globals. `createAgentSession({ capabilities })` replaces any subset; `createFakeCapabilities()` gives deterministic implementations. The default credential capability holds nothing, so a build with no keychain binding denies credential injection rather than falling back elsewhere.
- **An assembled-runtime replay harness.** `createReplayRuntime()` boots the production composition — real loader, real session orchestration, real tool and provider pipelines — against fake capabilities and a scripted provider that calls `onPayload`/`onResponse` exactly where a real provider does, and hands back the recorded requests, responses, and tool audits.
- Tool calls are audited when there is something to say. `orphus.tool.audit.v1` records the arguments a tool actually ran with after mutable hooks, which property paths a hook changed, and whether the call executed, was blocked, or was rejected — written when a hook changed the arguments, the call was blocked, or revalidation rejected it. A call that ran exactly what the model asked for writes nothing, since the assistant's own tool call is already the record. A failed audit write never takes down the tool it observes.
...
- **Mutable `tool_call` hook output is now schema-validated before policy and execution.** Extensions may still mutate `event.input` in place, but after the last handler returns the arguments are re-checked against the tool's own schema: a mutation that leaves them invalid becomes a durable tool error and the tool does not run. Everything downstream — policy, approval prompts, and the tool itself — sees the post-hook arguments and no earlier version, so an approval can no longer display one command while another one executes.
- **Extension resource registration is transactional.** Tools, commands, shortcuts, flags, providers, and flag defaults registered during an event now commit together or not at all, and transactions nest so only the outermost commit publishes. A handler that threw partway through used to publish whatever it had already registered, leaving a half-installed tool set indistinguishable from a successful one; that batch now rolls back completely, without applying the batch's own tool refresh or active-tool selection.
...
- `gate.model` is preflighted with every other model pin, so a misnamed gate model fails at validate time instead of after the entire pipeline has run.
- A deliberating team member whose explicit `tools` list omits `roundtable` (or is empty) now draws a validation warning — render instructs that member to join the room with a tool it cannot call.
- A regular file squatting on a fleets directory path reports "cannot list fleets" with the path, instead of an unhandled `ENOTDIR` escaping past the blueprint error handling.
- The roundtable broker socket now comes up owner-only (`0600`, directory `0700`). The docs promised a same-user trust boundary; umask was the only thing delivering it, and under `0022` any local user could connect and register under any name.
- A wedged roundtable startup now says why: when the broker child dies on a stale startup lock, its remove-and-retry message died with it (the child runs detached with stdio ignored) and callers saw only "did not start in time". The spawner post-mortems the lock file and names the stale lock, its dead owner, and the fix.
- A roundtable client whose registration times out destroys its socket instead of leaking it — a peer that accepted and never answered used to leave a ref'd handle holding the event loop open for good.
- A second `register` on one broker connection is refused instead of silently orphaning the first session as a ghost member — a ghost kept `sessions.size` above zero, which blocked the broker's idle exit permanently.
- The digest's collapse marker now gives advice that works: it names the exact `after_seq` to fetch. "Raise budget" was a dead end — the digest advances the read cursor past collapsed messages, so the suggested re-digest returned "No new messages".
- The in-app changelog viewer no longer rewrites upstream attribution links onto this repository. pi-mono and Atomic issue/PR references pass through verbatim — their numbers do not exist here, so canonicalizing them manufactured dead links — while package-relative documentation links still resolve to this repository pinned at the release tag.
- OpenRouter attribution now identifies Orphus: the `HTTP-Referer` header points at this repository instead of Atomic's site, matching the already-rebranded title, category, NVIDIA billing-origin, and OpenCode client headers.
- `orphus-roles --format orca` honors an explicit role `cwd` (via `cd` in the terminal command) instead of silently dropping it; a defaulted cwd still leaves the directory to the worktree, which is the point of `--worktree`.
- `orphus update` on an archive install now updates *that* install. The self-update one-liner carried no environment, so a custom `ORPHUS_INSTALL_DIR` install got a full second copy at the default paths, a success message, and its real install untouched — with the version nag repeating forever. The command now pins `ORPHUS_INSTALL_DIR` to the running binary's own root, and the manual guidance printed by `orphus update`'s fallback does the same.
- A prerelease install can now track prereleases through `orphus update`: the resolved release version is passed to the installer as `ORPHUS_VERSION`, instead of the installer re-resolving `/releases/latest` — which GitHub redirects to the newest non-prerelease, silently downgrading every beta runner.
- Self-update fetches `install.sh` from the resolved release tag rather than main HEAD, so the installer that runs is the one that release shipped.
- A rate-limited or failing stable-channel check reports "could not determine" instead of falling through to the any-kind release list and offering a stable install the newest beta. Only a 404 — no stable release exists yet — falls through.
- The two version-check requests no longer share one `AbortSignal.timeout`: the shared budget could leave the fallback request nothing, aborting exactly the path the fallthrough exists to serve.
- The installer prunes old versions: the just-installed release plus the one `current` pointed at before the update are kept — which is what "kept for rollback" means — instead of every update permanently adding a full release to the install root.
- `count` + `name` on one subagent task now yields distinctly named children (`reviewer-1`, `reviewer-2`, …) instead of N children sharing one identity. The roundtable broker keys read cursors and message attribution on the session name, so shared-name siblings became one member: every digest marked messages read for all of them, and their posts merged.
...
- The release installer no longer prints an expected 404 on every run, and it names a full disk as a full disk. While only prereleases exist, the `/releases/latest` probe 404s by design and the release list answers instead — but curl reported that 404 on stderr on every install and update. That one probe, the only one with a fallback behind it, is now quiet; a pinned `--ref` and the fallback itself stay loud. A download that dies writing to a full volume reported "failed to download release asset", sending the user after a network problem that was not there. A write failure (curl 23, wget 3) now names the volume, and curl 56 — which curl returns both for a dropped connection and for a short write to a full destination — names both without ranking them.

- `orphus inspect runtime` now honours project trust instead of assuming it: loading project-local extensions executes them, so inspection resolves the same remembered trust decisions as a real session and — having no UI to ask — treats an undecided project as untrusted rather than trusted-because-a-report-wanted-it.
- Credential redaction in provider request records no longer stops at objects: anything string- or container-shaped under a credential-named key is redacted wholesale (`authorization: { value: … }` included), while numeric fields whose names merely resemble credentials — `max_tokens` in nearly every payload — are now kept instead of falsely redacted.
- Browser cleanup can no longer hang shutdown: killing Chrome escalates from SIGTERM to SIGKILL on a bounded grace period each, and a process that survives both is reported as a cleanup failure instead of awaited forever. A launch that fails mid-start also clears its stale DevTools endpoint, and a dropped connection settles event waiters immediately with the close reason rather than a late timeout.
- Login origins are real origins only: `file:`, `data:`, and custom-scheme pages all serialize their origin to the string `"null"`, and one such allowlist entry would have matched every one of them. Opaque origins are now rejected at the single choke point, and the credential registry drops entries with non-string usernames and duplicate labels at parse.
- `ProcessCapability.exec` resolves on stream close rather than process exit, so output written while the pipes drain — a grandchild holding stdout, most visibly — is no longer lost; and both `exec` and the terminal transport decode UTF-8 statefully, so a multi-byte character split across chunks arrives whole instead of as replacement characters.
- Browser login hardening: the page's origin is re-checked immediately before the form is filled, so a page that navigates away while the approval prompt is open receives nothing; a page-thrown error on the password field comes back with the secret redacted rather than page-authored text carrying it into a tool result; `ORPHUS_BROWSER_NO_SANDBOX` accepts only an explicit `1`/`true`/`on`/`yes` (previously `no` counted as yes); a keychain lookup that blocks on an unlock prompt is bounded instead of hanging the turn; and an origin denial no longer echoes the raw page URL, which can carry tokens.
- Browser lifecycle: `close()` during an in-flight `open` now waits for the launch and tears down the Chrome it registered instead of reporting an empty registry as success; connecting to a DevTools socket that never answers times out instead of hanging the `open` action, and an `open` whose signal was already aborted fails on the spot without opening a socket; a page attach that fails closes the target it created; and a page's event waits are scoped to its own target session.
- termDOM picker: a failed delete or rename reloads the list from disk so it cannot keep showing a state the filesystem does not have; emoji and other astral characters can be typed into the search query and rename draft (and backspace removes them whole); Shift+Tab encodes as backtab instead of firing Tab's binding; starting a second selection while one is live is refused loudly instead of orphaning the first renderer on stdin; and a rename lands in both scopes even when the same session appears under differently-formed paths.
- Transcription: Escape during first-run setup or a model download now cancels the pending start instead of being ignored while recording begins anyway, and the `initialize` handshake is bounded so a worker that opens and then never answers cannot stall the worker→helper fallback. Native artifact paths are derived with `fileURLToPath`, fixing Windows drive-letter paths.
- An invalid `ORPHUS_TUI_BACKEND` value now falls through to the `tui.backend` setting, as the precedence documentation always said, instead of eclipsing it; and a single astral-plane keystroke (an emoji in a session name) reaches the termDOM picker's query instead of being dropped.
- Transcription hardening: a backend that answers `ready` but ignores `dispose` can no longer stall the worker→helper fallback ladder (the acknowledgement wait is bounded); a malformed protocol message — unparsable, unknown, or a known kind with wrong field types — is terminal for that backend rather than read past; a helper stdin write racing the helper's exit closes the channel instead of crashing the host on EPIPE; a second `Ctrl+Alt+Z` during first-run setup can no longer start recording twice; model downloads fail cleanly when the disk does (stream errors reject instead of hanging or crashing); settings temp files use unpredictable, exclusively-created names; and a corrupt or future-versioned settings file is reported before setup reruns instead of silently discarded.

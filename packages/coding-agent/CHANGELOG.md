...
- Tool calls are audited. `orphus.tool.audit.v1` records the arguments a tool actually ran with after mutable hooks, which property paths a hook changed, and whether the call executed, was blocked, or was rejected. A failed audit write never takes down the tool it observes.

...
- The release installer no longer prints an expected 404 on every run, and it names a full disk as a full disk. While only prereleases exist, the `/releases/latest` probe 404s by design and the release list answers instead — but curl reported that 404 on stderr on every install and update. That one probe, the only one with a fallback behind it, is now quiet; a pinned `--ref` and the fallback itself stay loud. A download that dies writing to a full volume (curl 23, wget 3, or curl's short-write 56) reported "failed to download release asset", sending the user after a network problem that was not there; it now names the volume.

...
**Important:** Use `StringEnum` from `@orphus/coding-agent` for string enums. It retains Pi's Google-compatible schema and composes with Atomic's direct TypeBox types; `Type.Union`/`Type.Literal` doesn't work with Google's API.

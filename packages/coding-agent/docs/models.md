...
Orphus reads one `models.json` from the active agent directory. It does not layer model overrides from `.pi` and `.orphus` files.
...
Authenticated remote catalogs are cached in `models-store.json`. Orphus revalidates pi.dev catalogs with the stored ETag through `If-None-Match`; an empty `304 Not Modified` is success and retains the cached body while updating its check time. A newer bundled catalog wins over an older persisted overlay even when package file mtimes are misleading. Final visibility is built-ins, persisted/remote data subject to freshness, the single active-agent-directory `models.json`, and live provider catalogs/overrides. The `.pi`/`.orphus` layering described elsewhere applies to other configuration surfaces (`auth.json` among them), never to the model catalog. Provider failures retain the last usable provider-specific snapshot.

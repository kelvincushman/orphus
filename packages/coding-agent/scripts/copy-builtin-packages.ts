...
// Issue #1208: the @orphus/coding-agent/workflows types condition must resolve to
...
//
// Ambient bridge so the documented bare specifier \`@orphus/workflows\` (and its
// \`builtin/*\` submodules) type-check in installed packages that depend only on
// @orphus/coding-agent. Runtime resolution is unchanged: Atomic's jiti virtual-module
...
const WORKSPACE_BUILTINS = [
	{ packageName: "@orphus/workflows", workspaceDirName: "workflows" },
	{ packageName: "@orphus/subagents", workspaceDirName: "subagents" },
	{ packageName: "@orphus/mcp", workspaceDirName: "mcp" },
	{ packageName: "@orphus/web-access", workspaceDirName: "web-access" },
	{ packageName: "@orphus/intercom", workspaceDirName: "intercom" },
	{ packageName: "@orphus/roundtable", workspaceDirName: "roundtable" },
	{ packageName: "@orphus/fleet", workspaceDirName: "fleet" },
] as const;
...
// Emit the lean @orphus/workflows authoring surface (authoring.ts +
// shared/authoring-contract.ts) as declarations into the copied workflows tree so
// the @orphus/coding-agent/workflows exports `types` condition resolves to `.d.ts`.
function emitWorkflowAuthoringTypes(): void {
	...
	if (result.status !== 0) {
		throw new Error(`Failed to emit @orphus/workflows authoring declarations (tsgo exited ${result.status ?? "null"})`);
	}
	...
	console.log(`Emitted @orphus/workflows authoring types -> ${join("dist", "builtin", "workflows", "src", "authoring.d.ts")}`);
}
...
// specifier `@orphus/workflows` (and its `builtin/*` submodules) onto the
// @orphus/coding-agent subpath exports. Builtin names are derived from the copied
...
function writeWorkflowsAmbientDeclaration(): void {
	const builtinBlocks = listWorkflowBuiltinNames()
		.map(
			(name) =>
				`declare module "@orphus/workflows/builtin/${name}" {\n` +
				`\texport * from "@orphus/coding-agent/workflows/builtin/${name}";\n` +
				`\texport { default } from "@orphus/coding-agent/workflows/builtin/${name}";\n` +
				`}`,
		)
		.join("\n\n");
	const contents =
		`${ambientHeader}\n\n` +
		`declare module "@orphus/workflows" {\n\texport * from "@orphus/coding-agent/workflows";\n}\n\n` +
		`declare module "@orphus/workflows/builtin" {\n\texport * from "@orphus/coding-agent/workflows/builtin";\n}\n\n` +
		`${builtinBlocks}\n`;
	mkdirSync(workflowsDistDir, { recursive: true });
	writeFileSync(workflowsAmbientDts, contents, "utf-8");
	console.log(`Wrote @orphus/workflows ambient bridge -> ${join("dist", "builtin", "workflows", "ambient.d.ts")}`);
}

// Reference the ambient bridge from the package root declarations so packages that
// import @orphus/coding-agent anywhere (or use `types: ["@orphus/coding-agent"]`) pick up the
// `@orphus/workflows` ambient module automatically. Idempotent.
...
async function bundleWorkflowsExtension(): Promise<void> {
	...
	console.log(`Bundled @orphus/workflows extension -> ${join("dist", "builtin", "workflows", "src", "extension", "index.bundle.mjs")}`);
}

// Issue #1208: derive externally-resolvable @orphus/workflows types from source.

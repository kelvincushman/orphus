...
export function buildSearchDetails(
	base: SearchToolDetails | undefined,
	text: string,
	cwd: string,
	scopePath: string,
	missingPaths: string[] = [],
	...
	for (const line of text.split("\n")) {
		const header = line.match(/^\[([^\]#]+)#[0-9A-F]{4}\]$/);
		if (header) {
		...
		if (pathMatch) {
			currentPath = pathMatch[1] ?? currentPath;
			files.add(currentPath);
		}
		const isMatch = /^\*\d+:/.test(line) || /^.+?:\d+: /.test(line);
		if (isMatch && currentPath) {
			matchCount++;
			fileMatches[currentPath] = (fileMatches[currentPath] ?? 0) + 1;
		}
	}
	return {
		...(base ?? {}),
		...
		displayContent: text,
		meta: { ...(base?.meta ?? {}), source: scopePath, ...(base?.truncation ? { truncation: base.truncation } : {}) },
	};
}

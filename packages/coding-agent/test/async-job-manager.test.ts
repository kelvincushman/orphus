...
import {
	...
	createSessionAsyncJobManager,
	disposeSessionAsyncJobManager,
} from "../src/core/async/session-manager.ts";
...
function createCapturedSession(
	captured: CapturedCustomMessage[],
...
describe("AsyncJobManager", () => {
		...
		const bash = createBashToolDefinition(process.cwd(), {
			...
			operations: {
				exec: async (_command, _cwd, { onData }) => {
					onData(Buffer.from("session done\n"));
					return { exitCode: 0 };
				},
			},
		});
	...
	it("routes shared singleton jobs to the session that started each job", async () => {
		...
		const bash = createBashToolDefinition(process.cwd(), {
			...
			operations: {
				exec: async (_command, _cwd, { onData }) => {
					onData(Buffer.from("later session\n"));
					return { exitCode: 0 };
				},
			},
		});
	...
	it("keeps the shared manager alive when the owner disposes while a later session has an active job", async () => {
		...
		const bash = createBashToolDefinition(process.cwd(), {
			...
			asyncJobDeliveryHandler: createSessionAsyncDeliveryHandler(
				laterSession,
				laterHandle.manager,
				laterHandle.sessionId,
			),
			asyncJobSessionId: laterHandle.sessionId,
			operations: {
				exec: async (_command, _cwd, { onData }) => {
					await execFinished;
					onData(Buffer.from("later active\n"));
					return { exitCode: 0 };
				},
			},
		});
	...
	it("preserves a streaming delivery admitted before its session is disposed without blocking other sessions", async () => {
		...
		const makeBash = (textValue: string, handle: typeof staleHandle, session: typeof staleSession) =>
	...
	it("does not let one live streaming session block unrelated async delivery", async () => {
		...
		const liveHandle = createSessionAsyncJobManager(liveSession);
		const makeBash = (textValue: string, handle: typeof streamingHandle, session: typeof streamingSession) =>
			createBashToolDefinition(process.cwd(), {
				asyncEnabled: true,
				asyncJobManager: handle.manager,
				asyncJobDeliveryHandler: createSessionAsyncDeliveryHandler(session, handle.manager, handle.sessionId),
		...
		await makeBash("streaming blocks", streamingHandle, streamingSession).execute("bash-streaming-blocker", {
			command: "echo streaming blocks",
			async: true,
		});
		await waitFor(() => streamingCaptured.length === 1);
		await makeBash("live proceeds", liveHandle, liveSession).execute("bash-live-proceeds", {
			command: "echo live proceeds",
			async: true,
		});
	...
	it("keeps disposed running job suppression until completion so fallback delivery cannot occur", async () => {
		...
		const bash = createBashToolDefinition(process.cwd(), {
			...
			operations: {
				exec: async (_command, _cwd, { onData }) => {
					await execFinished;
					onData(Buffer.from("disposed complete\n"));
					return { exitCode: 0 };
				},
			},
		});
		...
		releaseExec?.();
		await waitFor(async () =>
			text(await bash.execute("bash-disposed-poll", { command: `__atomic_bash_job ${jobId}` })).includes(
				"disposed complete",
			),
		);
	...
	it("polling cannot retract a streaming delivery admitted at receipt", async () => {
		...
		const bash = createBashToolDefinition(process.cwd(), {
			...
			operations: {
				exec: async (_command, _cwd, { onData }) => {
					onData(Buffer.from("queued then polled\n"));
					return { exitCode: 0 };
				},
			},
		});
	...
	it("a delivery admitted before manager disposal still settles exactly once", async () => {
		...
		let settled = false;
		const delivery = Promise.resolve(handler(message)).then(() => {
			settled = true;
		});
		...
		await Promise.race([
			...
			new Promise((_resolve, reject) =>
				setTimeout(() => reject(new Error("delivery wait did not settle after dispose")), 100),
			),
		]);
	...
	it("enforces the running job bound", async () => {
		...
		await expect(bash.execute("bash-async-2", { command: "sleep", async: true })).rejects.toThrow(
	...
	it("suppresses auto-delivery after explicit async bash cancellation while preserving pollability", async () => {
		...
		await waitFor(async () =>
			text(await bash.execute("bash-poll", { command: `__atomic_bash_job ${jobId}` })).includes("failed"),
		);
		await new Promise((resolve) => setTimeout(resolve, 60));
		expect(delivered).toHaveLength(0);
		expect(manager.isDeliverySuppressed(jobId)).toBe(true);

...
async function createTempDir(): Promise<string> {
	const dir = await mkdtemp(join(tmpdir(), "atomic-bash-intercept-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("bash interceptor parity", () => {
	it("does not run an interceptor unless it is explicitly configured", async () => {
		...
		const output = text(
			await bash.execute("bash-1", { command: "echo local" }, undefined, undefined, {} as ExtensionContext),
		);
		expect(calls).toBe(0);
	...
	it("honors cwd/env inputs and built-in interception when enabled", async () => {
		...
		const bash = createBashToolDefinition(dir, {
			...
			operations: {
				exec: async (_command, cwd, { onData, env }) => {
					seenCwd = cwd;
					seenEnv = env?.VALUE ?? "";
		...
		await expect(
			bash.execute("bash-block", { command: "cat file.txt" }, undefined, undefined, {} as ExtensionContext),
		).rejects.toThrow(/Use the read tool/);
		const output = text(
			await bash.execute(
				"bash-cwd",
				{ command: "echo ok", cwd: "sub", env: { VALUE: "set" } },
				undefined,
				undefined,
				{} as ExtensionContext,
			),
		...
		await expect(
			bash.execute(
				"bash-bad-env",
				{ command: "echo ok", env: { "BAD-NAME": "x" } },
				undefined,
				undefined,
				{} as ExtensionContext,
			),
	...
	it("does not block rules for unavailable tools and rejects unsupported runtime modes", async () => {
		...
		expect(
			text(
				await createBashToolDefinition(dir).execute(
					"bash-pty",
					{ command: "echo hi", pty: true },
					undefined,
					undefined,
	...
	it("checks bash interceptor rules before internal URL expansion", async () => {
		...
		const bash = createBashToolDefinition(dir, {
			interceptorEnabled: true,
			interceptorRules: [{ pattern: "artifact://secret", tool: "read", message: "raw internal URL blocked" }],
			operations: {
				exec: async (_command, _cwd, { onData }) => {
					onData(Buffer.from("unexpected"));
					return { exitCode: 0 };
				},
			},
		});
		const ctx = {
			resolveInternalUrl: (url: string) => (url === "artifact://secret" ? join(dir, "secret.txt") : undefined),
		};
		await expect(
			bash.execute(
				"bash-raw-intercept",
				{ command: "echo artifact://secret" },
				undefined,
				undefined,
				ctx as ExtensionContext,
			),
		).rejects.toThrow(/raw internal URL blocked/);
	});
	...
	it("checks built-in interceptor rules after internal URL expansion", async () => {
		...
		const bash = createBashToolDefinition(dir, {
			interceptorEnabled: true,
			interceptorRules: [{ pattern: "secret\\.txt", tool: "read", message: "expanded internal URL blocked" }],
			operations: {
				exec: async (_command, _cwd, { onData }) => {
					onData(Buffer.from("unexpected"));
					return { exitCode: 0 };
		...
		await expect(
			bash.execute(
				"bash-expanded-intercept",
				{ command: "echo artifact://secret" },
				undefined,
				undefined,
				ctx as ExtensionContext,
			),
		).rejects.toThrow(/expanded internal URL blocked/);
	});
	...
	it("surfaces invalid custom interceptor regexes", async () => {
		...
		await expect(
			bash.execute("bash-invalid-rule", { command: "echo safe" }, undefined, undefined, {} as ExtensionContext),
		).rejects.toThrow(/Invalid bash interceptor rule/);
	});
	it("expands internal URLs in command cwd and env before execution", async () => {
		...
		await bash.execute(
			"bash-url",
			{ command: "cat artifact://cmd", cwd: "local://sub", env: { VALUE: "artifact://env" } },
			undefined,
			undefined,
			ctx as ExtensionContext,
		);

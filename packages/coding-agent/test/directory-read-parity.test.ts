...
async function tempDir(): Promise<string> {
	const dir = join(tmpdir(), `atomic-dir-read-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
describe("directory read parity", () => {
	it("renders directory trees by recency with per-directory caps", async () => {
		...
		for (let index = 1; index <= 14; index++)
			await writeFile(join(dir, "many", `f${String(index).padStart(2, "0")}.txt`), "x");
		const output = text(
			await createReadToolDefinition(dir).execute("read-dir", { path: "." }, undefined, undefined, {} as never),
		);

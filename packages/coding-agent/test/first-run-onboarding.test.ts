...
describe("first-run onboarding", () => {
	...
	it("gates first-run onboarding on an empty started session and missing onboardedVersion", () => {
		...
		expect(
			isEligible.call({
				...
				settingsManager: { ...settingsManager, getOnboardedVersion: () => "0.1.0" },
				options: {},
			}),
		).toBe(false);
		expect(
			isEligible.call({
				session: { state: { messages: [] } },
				settingsManager: { ...settingsManager, getFirstRunOnboardingStartedVersion: () => undefined },
				options: {},
			}),
		).toBe(false);
		expect(
			isEligible.call({
				session: { state: { messages: [] } },
				settingsManager,
				options: { initialMessage: "run once" },
			}),
		).toBe(false);
		expect(
			isEligible.call({
	...
	it("uses a separate onboarding-start marker after changelog startup records the current version", () => {
		...
		const getChangelogForDisplay = Reflect.get(InteractiveMode.prototype, "getChangelogForDisplay") as (
			this: typeof host,
		) => string | undefined;
		const isEligible = Reflect.get(InteractiveMode.prototype, "isFirstRunOnboardingEligible") as (
			this: typeof host,
		) => boolean;
		...
		expect(
			isEligible.call({ ...host, settingsManager: upgraded, hadLastChangelogVersionAtStartup: true, options: {} }),
		).toBe(false);
	});
	...
	it("auto-completes onboarding for returning users with prior changelog state", () => {
		...
		const initialize = Reflect.get(InteractiveMode.prototype, "initializeFirstRunOnboardingMarkers") as (
			this: typeof host,
		) => void;
	...
	it("starts onboarding for fresh installs but does not activate it for initial input runs", () => {
		...
		const isEligible = Reflect.get(InteractiveMode.prototype, "isFirstRunOnboardingEligible") as (
			this: typeof host & { options: { initialMessage?: string } },
		) => boolean;
	...
	it("renders first-run notice without changelog markdown", () => {
		...
		const showStartupNoticesIfNeeded = Reflect.get(InteractiveMode.prototype, "showStartupNoticesIfNeeded") as (
			this: typeof host,
		) => void;
	...
	it("renders first-run notice after the changelog so it stays closest to the input", () => {
		...
		expect(
			host.chatContainer.children.slice(
				firstNoticeIndex,
				firstNoticeIndex + host.firstRunOnboardingNoticeComponents.length,
			),
		).toEqual(host.firstRunOnboardingNoticeComponents);
		expect(setOnboardedVersion).toHaveBeenCalledWith("0.2.0");
		expect(host.ui.requestRender).toHaveBeenCalledTimes(1);
	});
	...
	it("clears stale first-run notice state when starting a new session", async () => {
		...
		const handleClearCommand = Reflect.get(InteractiveMode.prototype, "handleClearCommand") as (
			this: typeof host & { clearFirstRunOnboardingUi: () => void },
		) => Promise<void>;

...
async function tempDir(): Promise<string> {
	const dir = await mkdtemp(join(tmpdir(), "atomic-hashline-recovery-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("hashline recovery and read selector parity", () => {
	it("recovers hashline edits across non-overlapping external drift", async () => {
		const dir = await tempDir(),
			...
			file = join(dir, "stale.txt");
		await writeFile(file, "one\ntwo\n", "utf8");
		const read = createReadToolDefinition(dir, { hashlineStore: store });
		const tag = text(
			await read.execute("read", { path: "stale.txt" }, undefined, undefined, {} as ExtensionContext),
		).match(/#([0-9A-F]{4})/)?.[1];
		expect(tag).toBeTruthy();
		await writeFile(file, "one\ntwo\nthree\n", "utf8");
		const output = text(
			await createEditToolDefinition(dir, { hashlineStore: store }).execute(
				"edit",
				{ input: `[stale.txt#${tag}]\nreplace 1..1:\n+ONE` },
				undefined,
				undefined,
	...
	it("rejects separated hunks against an in-place external modification (re-read required)", async () => {
		const dir = await tempDir(),
			store = createHashlineSnapshotStore(),
			file = join(dir, "multi.txt");
		await writeFile(file, "one\ntwo\nthree\nfour\nfive\nsix\n", "utf8");
		const read = createReadToolDefinition(dir, { hashlineStore: store });
		const tag = text(
			await read.execute("read", { path: "multi.txt" }, undefined, undefined, {} as ExtensionContext),
		).match(/#([0-9A-F]{4})/)?.[1];
		...
		await writeFile(file, "one\ntwo\nTHREE\nfour\nfive\nsix\n", "utf8");
		await expect(
			createEditToolDefinition(dir, { hashlineStore: store }).execute(
				"edit",
				{ input: `[multi.txt#${tag}]\nreplace 1..1:\n+ONE\nreplace 6..6:\n+SIX` },
				undefined,
				undefined,
	...
	it("recovers hashline edits shifted by external insertions above", async () => {
		const dir = await tempDir(),
			store = createHashlineSnapshotStore(),
			file = join(dir, "shifted.txt");
		await writeFile(file, "a\nb\nc\n", "utf8");
		const read = createReadToolDefinition(dir, { hashlineStore: store });
		const tag = text(
			await read.execute("read", { path: "shifted.txt" }, undefined, undefined, {} as ExtensionContext),
		).match(/#([0-9A-F]{4})/)?.[1];
		await writeFile(file, "x\na\nb\nc\n", "utf8");
		await createEditToolDefinition(dir, { hashlineStore: store }).execute(
			"edit",
			{ input: `[shifted.txt#${tag}]\nreplace 3..3:\n+C` },
			undefined,
			undefined,
			{} as ExtensionContext,
		);

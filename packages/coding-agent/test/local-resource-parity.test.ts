...
async function tempDir(): Promise<string> {
	const dir = await mkdtemp(join(tmpdir(), "atomic-local-resource-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("local resource parity", () => {
	it("uses source file labels and hashline snapshots for local:// read/search/write", async () => {
		const dir = await tempDir(),
			store = createHashlineSnapshotStore();
		await writeFile(join(dir, "a.txt"), "needle\n", "utf8");
		expect(
			text(
				await createReadToolDefinition(dir, { hashlineStore: store }).execute(
					"read-local",
					{ path: "local://a.txt" },
					undefined,
					undefined,
		...
		const search = await createSearchToolDefinition(dir, { hashlineStore: store }).execute(
			"search-local",
			{ pattern: "needle", paths: "local://a.txt" },
			undefined,
			undefined,
			{} as never,
		);
		...
		const write = await createWriteToolDefinition(dir, { hashlineStore: store }).execute(
			"write-local",
			{ path: "local://a.txt", content: "updated\n" },
			undefined,
			undefined,
			{} as never,
		);
	...
	it("rejects local resource traversal outside the workspace", async () => {
		...
		await expect(
			createWriteToolDefinition(dir).execute(
				"write-local-traversal",
				{ path: "local://../secret.txt", content: "nope" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("rejects symlink and selector escapes for local resources", async () => {
		...
		await expect(
			createReadToolDefinition(dir).execute(
				"read-archive-escape",
				{ path: "../outside/archive.zip:a.txt" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("reports write executable and resource metadata", async () => {
		const dir = await tempDir();
		const script = await createWriteToolDefinition(dir).execute(
			"write-script",
	...
	it("emits find progress and rich details", async () => {
		const dir = await tempDir(),
			updates: string[] = [];
		let pattern = "";
		const find = createFindToolDefinition(dir, {
			operations: {
				exists: () => true,
				stat: () => ({ isFile: false, isDirectory: true }),
				glob: (globPattern) => {
					pattern = globPattern;
					return [join(dir, "a.txt")];
				},
			},
		});
	...
	it("returns direct grep count and filesWithMatches mode output", async () => {
		...
		expect(
			text(
				await grep.execute(
					"grep-count",
					{ pattern: "needle", mode: "count", offset: 1 },
					undefined,
					undefined,
					{} as never,
				),
		...
		expect(
			text(
				await grep.execute(
					"grep-files",
					{ pattern: "needle", mode: "filesWithMatches" },
					undefined,
					undefined,
					{} as never,
				),
		...
		expect(
			text(
				await grep.execute(
					"grep-file-offset",
					{ pattern: "needle", path: "a.txt", mode: "filesWithMatches", offset: 1 },
					undefined,
					undefined,
					{} as never,
				),
	...
	it("matches reference schema edge constraints", async () => {
		...
		const skip =
			(createSearchToolDefinition(dir).parameters as { properties: { skip: { anyOf?: unknown[] } } }).properties.skip
				.anyOf ?? [];

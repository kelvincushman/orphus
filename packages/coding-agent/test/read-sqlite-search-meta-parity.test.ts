...
async function tempDir(): Promise<string> {
	const dir = await mkdtemp(join(tmpdir(), "atomic-read-sqlite-parity-"));
	tempDirs.push(dir);
	return dir;
}
afterEach(async () => {
...
describe("sqlite and metadata parity", () => {
	it("uses query default limit 20 and caps to 500", async () => {
		...
		try {
			...
			for (let i = 1; i <= 600; i++) db.run("insert into t values (?, ?)", i, `row${i}`);
		} finally {
			db.close();
		}
		const read = createReadToolDefinition(dir);
		const defaulted = text(
			await read.execute("default-limit", { path: "data.sqlite:t?order=id" }, undefined, undefined, {} as never),
		);
		expect((defaulted.match(/"id":/g) ?? []).length).toBe(20);
		const capped = text(
			await read.execute(
				"capped-limit",
				{ path: "data.sqlite:t?limit=1000&order=id" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("lists tables with row counts", async () => {
		...
		const db = new mod.Database(dbPath);
		try {
			db.run("create table alpha (id integer primary key)");
			db.run("create table beta (id integer primary key)");
		} finally {
			db.close();
		}
		const listing = text(
			await createReadToolDefinition(dir).execute(
				"list-tables",
				{ path: "data.sqlite" },
				undefined,
				undefined,
	...
	it("caps raw q rows and exposes read metadata", async () => {
		...
		try {
			...
			for (let i = 1; i <= 1500; i++) db.run("insert into t values (?)", i);
		} finally {
			db.close();
		}
		const raw = text(
			await createReadToolDefinition(dir).execute(
				"raw-q",
				{ path: "data.sqlite?q=select * from t" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await expect(
			createReadToolDefinition(dir).execute(
				"raw-attach",
				{ path: "data.sqlite?q=ATTACH DATABASE 'x' AS x" },
				undefined,
				undefined,
				{} as never,
			),
		).rejects.toThrow(/Invalid raw SQLite query/);
		await expect(
			createReadToolDefinition(dir).execute(
				"raw-master",
				{ path: "data.sqlite?q=select * from sqlite_master" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await expect(
			createReadToolDefinition(dir).execute(
				"raw-quoted-attach",
				{ path: 'data.sqlite?q=select * from "ATTACH"' },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("validates SQLite where clauses and write columns like oh-my-pi", async () => {
		...
		expect(
			text(
				await read.execute(
					"where-quoted",
					{ path: "data.sqlite:users?where=name%3D'LIMIT'" },
					undefined,
					undefined,
					{} as never,
				),
			),
		).not.toContain("Invalid SQLite where");
		await expect(
			read.execute(
				"where-intersect",
				{ path: "data.sqlite:users?where=1%3D1%20INTERSECT%20SELECT%201" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await expect(
			createWriteToolDefinition(dir).execute(
				"unknown-column",
				{ path: "data.sqlite:users:1", content: "{missing:'nope'}" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("propagates read/search/find meta details", async () => {
		...
		try {
			db.run("create table t (id integer primary key, body text)");
			db.run("insert into t values (1, 'needle')");
		} finally {
			db.close();
		}
		const sqliteRead = await createReadToolDefinition(dir).execute(
			"read-sqlite-meta",
			{ path: "data.sqlite:t" },
			undefined,
			undefined,
		...
		const search = await createSearchToolDefinition(dir).execute(
			"search-meta",
		...
		const find = await createFindToolDefinition(dir).execute(
			"find-meta",
			{ paths: ["."] },
			undefined,
			undefined,

...
function zipWithDataDescriptor(): Buffer {
	const name = Buffer.from("old.txt"),
...
describe("resource selector hardening", () => {
	it("rejects raw SQLite pragma table-valued functions and quoted sqlite internals", async () => {
		...
		await expect(
			read.execute(
				"raw-pragma-tvfc",
				{ path: "data.sqlite?q=select * from pragma_table_info('t')" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await expect(
			read.execute(
				"raw-sqlite-splice",
				{ path: 'data.sqlite?q=select "sqlite""_master" from t' },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("rejects SQLite where filters that contain subqueries or internal functions", async () => {
		...
		await expect(
			read.execute(
				"where-subselect",
				{ path: "data.sqlite:t?where=exists(select%201)" },
				undefined,
				undefined,
				{} as never,
			),
		).rejects.toThrow(/Invalid SQLite where filter/);
		await expect(
			read.execute(
				"where-load-extension",
				{ path: "data.sqlite:t?where=load_extension('x')" },
				undefined,
				undefined,
				{} as never,
			),
		...
		await expect(
			read.execute(
				"where-sqlite-internal",
				{ path: "data.sqlite:t?where=sqlite_master%3D1" },
				undefined,
				undefined,
				{} as never,
			),
	...
	it("rejects malformed zip central directory offsets during selective writes", async () => {
		const dir = await tempDir();
		const archivePath = join(dir, "bad.zip");
		const eocd = Buffer.alloc(22);
		eocd.writeUInt32LE(0x06054b50, 0);
		eocd.writeUInt16LE(1, 10);
		eocd.writeUInt32LE(1000, 16);
		await writeFile(archivePath, eocd);
		await expect(
			createWriteToolDefinition(dir).execute(
				"bad-zip-write",
				{ path: "bad.zip:new.txt", content: "new" },
				undefined,
				undefined,
				{} as never,
			),
		).rejects.toThrow(/Invalid zip (archive|entry bounds)/);
	});
	...
	it("rejects selective zip writes that would drop data descriptors", async () => {
		...
		await expect(
			createWriteToolDefinition(dir).execute(
				"descriptor-zip-write",
				{ path: "descriptor.zip:new.txt", content: "new" },
				undefined,
				undefined,
				{} as never,
			),
		).rejects.toThrow(/Unsupported zip data descriptor/);
	});
});

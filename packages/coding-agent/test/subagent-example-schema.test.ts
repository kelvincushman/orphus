...
import { MAX_PARALLEL_TASKS, SubagentParams } from "../examples/extensions/subagent/schemas.js";
...
describe("subagent example parallel task limit", () => {
	test("accepts 50 parallel tasks and rejects 51", () => {
		...
		expect(Value.Check(SubagentParams, { tasks: makeTasks(MAX_PARALLEL_TASKS) })).toBe(true);
		expect(

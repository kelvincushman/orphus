...
- **A chain step's `{outputs.name}` splice is bounded rather than total.** It used to substitute the previous step's complete text into the next prompt: a step emitting 100 KB put 100 KB into its successor, with nothing between the two — the weakest boundary in the system. It now splices 2000 characters plus the path to the full output, through the same `boundedRender` core the room digest and parallel returns use. **`{outputs.name.full}` is the explicit opt-out** for a step that genuinely needs everything, and exists because bounding without one would break chains written when the splice was total. As with parallel returns, an output with no artifact path to point at splices in full rather than discarding what it cannot relocate. A reference with any other suffix — `{outputs.name.raw}` — is now rejected up front rather than silently splicing something the author did not ask for.
- **A parallel fan-out's return is now bounded by the runtime rather than truncated.** Every child's output used to be concatenated into the parent's context in full, guarded only by a blunt 200 KB / 5000-line cap that fires long after the parent's context is the scarce resource. Parallel returns now render through the same tiering the room digest uses — the shared `boundedRender` core, so the two boundaries cannot drift into disagreeing about what bounded means: a budget spent on what matters, the rest degraded to one-line headlines, and anything left collapsed to a count. Four children emitting 100 KB each reach the parent as roughly 1.7 KB, with every task named and every line carrying the path to its full output on disk. Failures are ordered first, so a silently collapsed error — a summary that looks fine while something broke — cannot happen. `outputMode: "inline"` opts back into the old full-output behaviour per call, for the small outputs where it is genuinely better.
...
- Subagent tasks (SINGLE, PARALLEL `tasks[]`, chain parallel steps) accept an optional `tools` array that replaces the agent definition's allowlist for that task, mirroring per-call `skill` semantics. The admitted child policy already honored spec-level tools; this exposes it at the tool surface so orchestrators (fleets in particular) can grant capabilities like `roundtable` per seat.

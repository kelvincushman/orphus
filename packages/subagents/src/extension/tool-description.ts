...
• PARALLEL: { tasks: [{agent,task,count?,output?,reads?,handoff?,cheapestFirst?,progress?}, ...], concurrency?: number, worktree?: true } - independent concurrent tasks (worktree: isolate each task in a git worktree)
...
• handoff: { key: "value", ... } (SINGLE and PARALLEL items) - small parent-known facts rendered at the top of the child task within a ~2000-char bound and labelled asserted-not-verified; put large content in a file and use reads. Pair with context:"fresh" to give a cheap model exactly what it needs without forking the parent transcript
• cheapestFirst:true (SINGLE and PARALLEL items) - start the agent's model ladder at its cheapest priced rung and escalate on failure

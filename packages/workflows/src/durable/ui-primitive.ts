...
export function wrapUiWithDurable(base: WorkflowUIContext, deps: DurableUiDeps): WorkflowUIContext {
	...
	const record = async (
		...
		response: WorkflowSerializableValue,
	): Promise<void> => {
	...
	const cached = (identity: { readonly hash: string }): WorkflowSerializableValue | undefined =>
		deps.backend.getUiResponse(deps.workflowId, identity.hash);

	const cachedCustom = (identity: {
		readonly hash: string;
	}): { readonly found: boolean; readonly response?: WorkflowSerializableValue } => {
		const hit = deps.backend
			.listCheckpoints(deps.workflowId)
			.find(
				(checkpoint) =>
					checkpoint.kind === "ui" &&
	...
	const waitForPendingWrite = async (
		...
		try {
			...
			try {
				...
				/* Preserve the opening-write failure. */
			}
			throw error;
		}
	};
	...
	return {
		async input(promptText: string): Promise<string> {
			...
			try {
				...
				await releasePending(identity, pending.token);
			}
		},

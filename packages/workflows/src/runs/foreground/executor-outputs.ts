...
import { isWorkflowExitSnapshotInvalidValue, workflowExitSnapshotInvalidValueMessage } from "./executor-abort.js";

function hasOwnWorkflowOutput(
	record: WorkflowOutputValues | Readonly<Record<string, WorkflowOutputSchema>>,
	key: string,
): boolean {
...
export function normalizeWorkflowRunOutput(workflowName: string, rawOutput: unknown): WorkflowOutputValues | undefined {
...
export function normalizeWorkflowExitOutput(
	...
	if (isWorkflowExitSnapshotInvalidValue(snapshot.value)) {
		...
		throw new Error(
			`atomic-workflows: ${invalidMessage ?? `workflow "${workflowName}" ctx.exit() outputs must be ${WORKFLOW_SERIALIZABLE_DESCRIPTION}, got object`}`,
		);
	}
...
export function assertWorkflowRunOutputs(
	...
	assertWorkflowOutputsExplicit(`workflow "${workflowName}"`, result ?? {}, declaredOutputs ?? {});
}

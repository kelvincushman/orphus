...
export function actionableReturnedStatusText(result: WorkflowOutputValues | undefined): string | undefined {
	return (
		stringResultField(result, "summary") ??
		stringResultField(result, "remaining_work") ??
		stringResultField(result, "result")
	);
}
...
export function structuredRecoverableWorkflowFailure(
	...
	return failedStage === undefined ? undefined : structuredRecoverableFailureFromSnapshot(failedStage, failedStage.id);
}
...
function isRecoverableProviderFailureKind(
	kind: WorkflowFailureKind | undefined,
): kind is "auth" | "rate_limit" | "provider" {

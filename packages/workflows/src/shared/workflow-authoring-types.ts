...
type UnionToIntersection<T> = (T extends T ? (value: T) => void : never) extends (value: infer TIntersection) => void
	? TIntersection
	: never;
...
type DeclaredResolvedEntry<K extends string, S extends TSchema> =
	S extends TOptional<TSchema>
		? { readonly [P in K]?: Static<S> & WorkflowSerializableValue }
		: { readonly [P in K]: Static<S> & WorkflowSerializableValue };

type DeclaredProvidedEntry<K extends string, S extends TSchema> = S extends
	| TOptional<TSchema>
...
export type WorkflowInputsFromSchemas<TSchemas extends WorkflowInputSchemaMap> = WorkflowInputShape<
	WorkflowResolvedInputShapeFromSchemas<TSchemas>
>;

export type WorkflowProvidedInputsFromSchemas<TSchemas extends WorkflowInputSchemaMap> = WorkflowInputShape<
	WorkflowProvidedInputShapeFromSchemas<TSchemas>
>;
...
export type WorkflowOutputsFromSchemas<TSchemas extends WorkflowOutputSchemaMap> = WorkflowOutputShape<
	WorkflowDeclaredOutputsFromSchemas<TSchemas>
>;

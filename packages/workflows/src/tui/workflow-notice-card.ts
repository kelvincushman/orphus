...
function appendHeadline(rows: string[], opts: WorkflowNoticeCardOpts, innerWidth: number): void {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		const styledPrefix = i === 0 ? style(rawPrefix, theme, tone, true) : " ".repeat(visibleWidth(rawPrefix));
...
function appendHint(rows: string[], hint: string, innerWidth: number, theme: GraphTheme | undefined): void {
	const prefix = "▸ ";
	const continuationPrefix = "  ";
	const budget = Math.max(1, innerWidth - 1 - visibleWidth(prefix));
	...
	for (let i = 0; i < lines.length; i++) {
		const rawPrefix = i === 0 ? prefix : continuationPrefix;
		const styledPrefix = i === 0 ? style(rawPrefix, theme, theme?.accent, true) : " ".repeat(visibleWidth(rawPrefix));
...
function colorForField(theme: GraphTheme | undefined, tone: WorkflowNoticeCardField["tone"]): string | undefined {

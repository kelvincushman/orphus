...
function cargoTargets(): VersionTarget[] {
	...
	const cargoLockTargets = existsSync(resolve(ROOT, "Cargo.lock"))
		? [{ kind: "cargoLock" as const, filePath: "Cargo.lock" }]
		: [];
...
async function bumpJsonFile(filePath: string, version: string): Promise<void> {
	...
	console.log(
		`  ${filePath}: ${oldVersion ?? "(none)"} → ${version}${dependencyChanges > 0 ? ` (${dependencyChanges} dependency range${dependencyChanges === 1 ? "" : "s"})` : ""}`,
	);
}
...
async function bumpReadme(filePath: string, version: string, optional = false): Promise<void> {
	...
	if (updated === content) {
		if (
			/https:\/\/img\.shields\.io\/badge\/version-[^"]+-blue/.test(content) ||
			/alt="Version [^"]+"/.test(content)
		) {

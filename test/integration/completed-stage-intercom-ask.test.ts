...
function session(sessionId: string, promptImpl: (text: string) => Promise<string>): StageSessionRuntime {
	...
	return {
		...
		async prompt(text: string) {
			lastAssistantText = await promptImpl(text);
...
async function waitForCompletedStage(
	...
	while (Date.now() < deadline) {
		for (const currentRun of store.runs()) {
			const stage = currentRun.stages.find(
				(candidate) => candidate.name === name && candidate.status === "completed",
			);
...
test("parallel workflow revives completed A for B's exact correlated ask and terminates normally", async () => {
	...
	assert.deepEqual(
		result.stages.filter((stage) => stage.name === "A").map((stage) => stage.status),
		["completed"],
	);
});

...
describe("runtime tunables — defaultConcurrency", () => {
	...
	test("defaultConcurrency:2 allows up to 2 concurrent stages", async () => {
		...
		const wf = workflow({
			...
			outputs: {},
			run: async (ctx) => {
				await Promise.all(["s1", "s2", "s3", "s4"].map((n) => ctx.stage(n).prompt(n)));
				return {};
			},
		});
		const result = await run(
	...
	test("max_concurrency input overrides the default stage concurrency", async () => {
		...
		const wf = workflow({
			...
			outputs: {},
			run: async (ctx) => {
				await Promise.all(["s1", "s2", "s3", "s4", "s5"].map((n) => ctx.stage(n).prompt(n)));
				return {};
			},
		});
		const result = await run(
	...
	test("ctx.parallel concurrency option limits scheduled task fan-out", async () => {
		...
		let maxActive = 0;
		const wf = workflow({
			name: "rt-parallel-option-concurrency",
			description: "",
			inputs: {},
			outputs: {},
			run: async (ctx) => {
	...
	test("ctx.parallel failFast:false waits for all scheduled task fan-out", async () => {
		...
		const wf = workflow({
			name: "rt-parallel-fail-fast-false",
			description: "",
			inputs: {},
			outputs: {},
			run: async (ctx) => {
	...
	test("slot released after stage failure — next stage can acquire", async () => {
		const ran: string[] = [];
		const wf = workflow({
			name: "rt-conc-fail-release",
			description: "",
			inputs: {},
			outputs: {},
			run: async (ctx) => {
				await Promise.allSettled([ctx.stage("will-fail").prompt("fail"), ctx.stage("will-pass").prompt("pass")]);
				return {};
			},
		});
		const result = await run(

...
describe("runtime tunables — statusFile", () => {
	...
	test("statusFile:true writes status.json after run start", async () => {
		...
		const writer = createStatusWriter(s, baseConfig({ statusFile: true, statusFilePath: filePath }));
	...
	test("statusFile:false writes no file", async () => {
		...
		const writer = createStatusWriter(s, baseConfig({ statusFile: false, statusFilePath: filePath }));

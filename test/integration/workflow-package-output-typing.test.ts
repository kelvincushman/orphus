...
const workflowOutputFixture = `import { createStore, run, workflow } from "@orphus/workflows";
import type { RunResult, ToolNodeSnapshot } from "@orphus/workflows";
...
function writePackageFixture(fixtureRoot: string): void {
	...
	writeFileSync(
		...
		JSON.stringify({ name: "workflow-output-package-types", private: true, type: "module" }, null, 2),
	);
	writeFileSync(
...
function writeSourceFixture(fixtureRoot: string): void {
	...
	writeFileSync(
		join(fixtureRoot, "package.json"),
		JSON.stringify({ name: "workflow-output-source-types", private: true, type: "module" }, null, 2),
	);
	writeFileSync(
		join(fixtureRoot, "tsconfig.json"),
		JSON.stringify(

...
describe("multi-stage resume acknowledgement coherence", () => {
	test("settles every stage and reports partial running state before a later retry", async () => {
		...
		const outcome = resumeRun(runId, { store, stageControlRegistry: registry }).then(
			(result) => ({ result }),
			(error: unknown) => ({ error }),
		);
		...
		assert.deepEqual(
			first.result.resumed.map((stage) => stage.id),
			["resume-late"],
		);
		...
		assert.deepEqual(
			first.result.snapshot.stages.map((stage) => stage.status),
			["paused", "running"],
		);
		...
		if (retried.ok)
			assert.deepEqual(
				retried.resumed.map((stage) => stage.id),
				["reject-first"],
			);
		assert.deepEqual(
			store
	...
	test("waits for a late rejection after an earlier stage visibly resumes", async () => {
		...
		let settled = false;
		const operation = resumeRun(runId, { store, stageControlRegistry: registry }).finally(() => {
			settled = true;
		});
		await Promise.resolve();
		await Promise.resolve();
	...
	test("counts a late rejection as resumed after its stage is visibly running", async () => {
		...
		assert.deepEqual(
			result.snapshot.stages.map((stage) => stage.status),
			["running", "running"],
		);
	...
	test("surfaces durable running transition and flush failures after visible resume", async () => {
		...
		class ThrowFlushBackend extends InMemoryDurableBackend {
			async flush(): Promise<void> {
				throw new Error("durable running flush failed");
			}
		}
	...
	test("retries a failed durable running transition after local resume already succeeded", async () => {
		...
		if (first.ok) {
			...
			assert.deepEqual(
				first.resumed.map((stage) => stage.id),
				["only"],
			);
	...
	test("successful resume preserves target order despite reverse settlement", async () => {
		...
		for (const [stageId, acknowledgement] of [
			["stage-a", a],
			["stage-b", b],
		] as const) {
			const state: ResumeHandleState = { status: "paused", calls: 0 };

...
import { inspectRun, killAllRuns, killRun, resumeRun } from "../../packages/workflows/src/runs/background/status.js";
...
describe("killAllRuns — persistence", () => {
	test("appends one workflow.run.end per in-flight run", () => {
		...
		assert.equal(
			calls.every((c) => c.type === "workflow.run.end"),
			true,
		);
		assert.equal(
			calls.every((c) => c.payload.status === "killed"),
			true,
		);

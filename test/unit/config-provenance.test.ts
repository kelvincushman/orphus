...
import { loadWorkflowConfig, toScopedDiscoveryConfig } from "../../packages/workflows/src/extension/config-loader.js";
 ...
 */
function writeConfigFile(base: string, scope: "global" | "project", content: object): string {
...
describe("toScopedDiscoveryConfig — project override changes scope", () => {
	test("shared key: project wins → entry in projectWorkflows, absent from globalWorkflows", () => {
		...
		assert.equal(
			...
			join(homeDir, ".atomic", "agent", "../../packages/workflows/src/extension/g-only.ts"),
		);
	});
...
describe("discoverWorkflows — settings-project vs settings-global source kinds via toScopedDiscoveryConfig", () => {
	test("global workflow only → source kind is settings-global", async () => {
		...
		const dc = toScopedDiscoveryConfig({ workflows: { "global-wf-kind-test": { path: wfPath } } }, null, {
			homeDir: home,
			projectRoot: proj,
		});
	...
	test("disjoint global + project keys → distinct kinds in sources", async () => {
		...
		const projWfPath = writeWorkflowFile(join(proj, "workflows"), "Project Distinct", "disjoint-project");
		const dc = toScopedDiscoveryConfig(
	...
	test("end-to-end: loadWorkflowConfig + toScopedDiscoveryConfig + discoverWorkflows", async () => {
		...
		const projWfPath = writeWorkflowFile(join(proj, "workflows"), "E2E Project", "e2e-project-wf");

...
import {
	cleanupDiscoveryTempDirs,
	existsSync,
	join,
	makeTempDir,
	mkdirSync,
	writeFileSync,
	writeInvalidWorkflowJs,
	writeMissingSentinelWorkflowJs,
	writeNoStageWorkflowJs,
	writeWorkflowJs,
...
describe("discoverWorkflows — INVALID_DEFINITION diagnostics", () => {
	...
	test("forged __piWorkflow object emits workflow diagnostic", async () => {
		...
		const { registry, errors } = await discoverWorkflows({
			cwd,
			homeDir: makeTempDir("empty-forged"),
	...
	test("workflow that completes without creating stages registers structurally", async () => {
		...
		const { registry, errors } = await discoverWorkflows({
			cwd,
			homeDir: makeTempDir("empty-structural-no-stages"),
			includeBundled: false,
		});
	...
	test("discovery does not invoke workflow run bodies", async () => {
		...
		const { registry, errors } = await discoverWorkflows({
			cwd,
			homeDir: makeTempDir("empty-no-run-body-side-effects"),
			includeBundled: false,
		});
	...
	test("workflow that reaches a stage through an aliased primitive registers structurally", async () => {
		...
		const { registry, errors } = await discoverWorkflows({
			cwd,
			homeDir: makeTempDir("empty-aliased-stage-primitive"),
			includeBundled: false,
		});

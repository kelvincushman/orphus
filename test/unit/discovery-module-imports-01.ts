...
import { createProjectWorkflowFile, tmpRoot, validDefaultExportSrc } from "./discovery-module-imports-helpers.js";
...
export function registerDiscoveryModuleImportsSuite1(): void {
	...
	describe("scanWorkflowDir — supported file extensions", () => {
		test("discovers .js workflow files", async () => {
			await createProjectWorkflowFile("alpha.js", validDefaultExportSrc("Alpha", "alpha"));
		...
		test("discovers .mjs workflow files", async () => {
			await createProjectWorkflowFile("beta.mjs", validDefaultExportSrc("Beta", "beta"));
			const result = await discoverWorkflows({
				cwd: join(tmpRoot, "cwd"),
				homeDir: join(tmpRoot, "home"),
				includeBundled: false,
  ...
  `,
			...
			const importFailed = result.errors.some((e) => e.code === "IMPORT_FAILED" && e.source?.includes("gamma.cjs"));

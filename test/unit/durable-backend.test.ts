...
describe("InMemoryDurableBackend", () => {
	...
	test("keeps completed workflows out of resumable listing and in completed listing", () => {
		...
		assert.equal(backend.listResumableWorkflows().length, 0);
		assert.deepEqual(
			backend.listCompletedWorkflows().map((entry) => entry.workflowId),
			[WORKFLOW_ID],
		);
	});
...
describe("ctx.tool primitive (durable caching)", () => {
	...
	test("different args produce different cache keys", async () => {
		let callCount = 0;
		const tool = makeTool();
		await tool("fetch", { url: "a" }, async () => {
			callCount++;
			return "a-result";
		});
		await tool("fetch", { url: "b" }, async () => {
			callCount++;
			return "b-result";
		});
	...
	test("same-name same-args calls are distinct within one workflow run", async () => {
		let callCount = 0;
		const tool = makeTool();
		const first = await tool("send-email", { to: "a@example.com" }, async () => {
			callCount++;
			return "sent-1";
		});
		const second = await tool("send-email", { to: "a@example.com" }, async () => {
			callCount++;
			return "sent-2";
		});
	...
	test("same-name same-args calls replay by ordinal after resume", async () => {
		let callCount = 0;
		const tool1 = makeTool();
		await tool1("send-email", { to: "a@example.com" }, async () => {
			callCount++;
			return "sent-1";
		});
		await tool1("send-email", { to: "a@example.com" }, async () => {
		...
		await assert.rejects(
			() =>
				tool(
					...
					{ retriesAllowed: true, maxAttempts: 2, intervalMs: 1 },
				),
			/permanent/,
		);
	});
	...
	test("throws if cancelled", async () => {
		cancelled = true;
		const tool = makeTool();
		await assert.rejects(() => tool("post-cancel", {}, async () => "never"), /cancelled/);
	});

...
describe("DbosDurableBackend hydration (fresh process)", () => {
	...
	test("fresh hydration restores an inspection-only throwing failure and root attribution", async () => {
		...
		assert.ok(entry);
		assert.equal(
			fresh.listCompletedWorkflows().some((candidate) => candidate.workflowId === workflowId),
			false,
		);
		...
		const completedEntry = afterSuccess
	...
	test("hydrateWorkflow reconstructs stage checkpoints from DBOS envelopes", async () => {
		const cp: DurableStageCheckpoint = {
			kind: "stage",
			workflowId: "wf-h3",
			checkpointId: "stage:r1",
			name: "build",
			replayKey: "stage:build:1",
			output: "done",
			completedAt: 1400,
			sessionFile: "/tmp/build.jsonl",
			startedAt: 1000,
			durationMs: 400,
		};
	...
	test("flush then fresh hydrate preserves deep run-aware stage topology", async () => {
		...
		const stages = [
			...
			},
			{
				checkpointId: "left",
				name: "left",
				replayKey: "workflow:child:1:left",
				stageId: "left",
				parentIds: [],
				run: childRun,
			},
			{
				checkpointId: "right",
				name: "right",
				replayKey: "workflow:child:1:right",
				stageId: "right",
				parentIds: [],
				run: childRun,
			},
			{
				checkpointId: "join",
				name: "join",
				replayKey: "workflow:child:1:join",
				stageId: "join",
		...
		assert.deepEqual(
			hydrated.map((checkpoint) => checkpoint.topology),
			stages.map((stage) => ({
				version: 1,

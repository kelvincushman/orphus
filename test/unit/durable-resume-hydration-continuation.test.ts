...
import { createStore, test as exTest, run, Type, workflow } from "./executor-shared.js";
...
exTest("live ctx.task stage metadata converges on task durable replay key", async () => {
	...
	const def = workflow({
		...
		run: async (ctx) => {
			const parallel = await ctx.parallel(
				[
					{ name: "completion-reviewer-1", prompt: "A" },
					{ name: "evidence-reviewer-1", prompt: "B" },
				],
				{ failFast: false },
			);
	...
	const taskCheckpoint = checkpoints.find(
		(checkpoint) => checkpoint.checkpointId === "task:stage:task:completion-reviewer-1:1",
	);
	const lifecycleCheckpoint = checkpoints.find(
		(checkpoint) => checkpoint.checkpointId === "stage:stage:task:completion-reviewer-1:1",
	);
...
exTest("fresh-store cached child replay restores nested parallel hierarchy", async () => {
	...
	assert.equal(
		(await run(parent, {}, { runId, store: createStore(), durableBackend: backend, adapters })).status,
		"completed",
	);

	const childTopology = backend
		.listCheckpoints(runId)
		.find(
			(checkpoint) =>
				checkpoint.kind === "stage" &&
				checkpoint.name === "left" &&
				checkpoint.topology?.run?.runName === "parallel-child",
		);
	assert.ok(childTopology, "child stage ownership must be persisted under the root durable run");
	const persistedRoot = backend
		.listCheckpoints(runId)
		.filter(
		...
		.map((checkpoint) => ({
			...
			parents: checkpoint.topology!.parentIds,
		}));
	...
	const replayed = await run(
		...
		{},
		{
			...
			adapters: {
				prompt: {
					prompt: async () => {
						throw new Error("cached work should not rerun");
					},
				},
			},
		},
	...
	assert.equal(
		resumedRoot.stages.find((stage) => stage.name === "after")?.replayedFromStageId,
		persistedAfter.stageId,
	);
...
exTest("mixed cached and live child resume remains inspectable after completion", async () => {
	...
	assert.deepEqual(
		graph.stages.map((item) => item.name),

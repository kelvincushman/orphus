...
import type {
	...
	WorkflowUIContext,
} from "../../packages/workflows/src/shared/authoring-contract-ui.js";
...
function makeBaseUi(overrides: Partial<WorkflowUIContext> = {}): WorkflowUIContext & { calls: Record<string, number> } {
	...
	return {
		...
		async select<T extends string>(_message: string, _options: readonly T[]): Promise<T> {
			calls.select++;
			return "opt-a" as T;
		},
		async editor(_initial?: string) {
			calls.editor++;
			return "edited-text";
		},
		async custom<T>(_factory: unknown, _options?: unknown): Promise<T> {
			calls.custom++;
			return "custom-result" as unknown as T;
		},
		...overrides,
	};
}
...
describe("wrapUiWithDurable", () => {
	...
	test("same prompt repeated in one run uses ordinal-specific cache keys", async () => {
		const base = makeBaseUi({
			async input() {
				base.calls.input++;
				return `answer-${base.calls.input}`;
			},
		});
		const ui = wrap(base);
	...
	test("replays identical descriptors by author callsite when scheduling reverses", async () => {
		...
		const replayBase = makeBaseUi({
			async input() {
				throw new Error("durable replay must not delegate");
			},
		});
	...
	test("select options participate in prompt identity", async () => {
		const base = makeBaseUi({
			async select<T extends string>(_message: string, options: readonly T[]) {
				base.calls.select++;
				return options[base.calls.select - 1] ?? options[0]!;
			},
		});
		const ui = wrap(base);
	...
	test.sequential("replays one checkpoint when process cwd changes", async () => {
		...
		const firstBase = makeBaseUi({
			async input() {
				firstBase.calls.input++;
	...
	test("same-named grandchildren beneath different parents own distinct pending reservations", async () => {
		const parentA = new ScopedDurableBackend(backend, {
			rootWorkflowId: WORKFLOW_ID,
			scopePrefix: "workflow:parent-a:1",
		});
		const parentB = new ScopedDurableBackend(backend, {
			rootWorkflowId: WORKFLOW_ID,
			scopePrefix: "workflow:parent-b:1",
		});
		const grandchildA = new ScopedDurableBackend(parentA, {
			rootWorkflowId: WORKFLOW_ID,
			scopePrefix: "workflow:shared-grandchild:1",
		});
		const grandchildB = new ScopedDurableBackend(parentB, {
			rootWorkflowId: WORKFLOW_ID,
			scopePrefix: "workflow:shared-grandchild:1",
		});
	...
	test("nested sibling custom prompts list and replay only their parent-prefixed checkpoints", async () => {
		...
		const firstA = makeBaseUi({
			async custom<T>() {
				firstA.calls.custom++;
				return "answer-a" as T;
			},
		});
		const firstB = makeBaseUi({
			async custom<T>() {
				firstB.calls.custom++;
				return "answer-b" as T;
			},
		});
		const makeNestedUi = (base: WorkflowUIContext, scoped: ScopedDurableBackend, workflowId: string) =>
			wrapUiWithDurable(base, {
				workflowId,
				backend: scoped,
				nextCheckpointId: createCheckpointIdGenerator(),
			});
		...
		assert.equal(
			await promptCustomAtStableCallsite(
				makeNestedUi(firstA, grandchildA, "grandchild-a"),
				factory,
				"shared-picker",
			),
			"answer-a",
		...
		assert.equal(
			await promptCustomAtStableCallsite(
				makeNestedUi(firstB, grandchildB, "grandchild-b"),
				factory,
				"shared-picker",
			),
			"answer-b",
		...
		const replayA = makeBaseUi({
			async custom<T>() {
				replayA.calls.custom++;
				return "wrong-a" as T;
			},
		});
		const replayB = makeBaseUi({
			async custom<T>() {
				replayB.calls.custom++;
				return "wrong-b" as T;
			},
		});
		assert.equal(
			await promptCustomAtStableCallsite(
				makeNestedUi(replayB, grandchildB, "grandchild-b-replay"),
				factory,
				"shared-picker",
			),
			"answer-b",
		...
		assert.equal(
			await promptCustomAtStableCallsite(
				makeNestedUi(replayA, grandchildA, "grandchild-a-replay"),
				factory,
				"shared-picker",
			),
			"answer-a",

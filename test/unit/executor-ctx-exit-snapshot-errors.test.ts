		...
		const def = workflow({
			...
			run: async (ctx) => {
				try {
					...
					try {
						tamper.reason = "tampered";
					} catch {
						/* frozen */
					}
					try {
						if (tamper.outputSnapshot?.value) tamper.outputSnapshot.value.count = 999;
					} catch {
						/* frozen */
					}
					try {
						if (tamper.outputSnapshot?.value?.nested) tamper.outputSnapshot.value.nested.value = 999;
					} catch {
						/* frozen */
					}
					throw signal;

...
describe("ctx.exit", () => {
		...
		const makeSession = (): StageSessionRuntime => {
			...
			return {
				...
				async closeWorkflowStageGeneration() {
					lifecycleEvents.push(`close:${promptName}`);
				},
				async abort() {
					resolvePrompt?.();
				},
		...
		const result = await run(
			...
			{},
			{
				...
				onStageEnd: (_runId, stage) => {
					lifecycleEvents.push(`end:${stage.name}`);
				},
			},
		);
	...
	test("stops queued parallel work after exit with failFast false and limited concurrency", async () => {
		...
		assert.equal(
			result.stages.some((stage) => stage.name === "queued-a" || stage.name === "queued-b"),
			false,
		);
	});
	...
	test("delayed post-exit stage and workflow calls do not create graph artifacts", async () => {
		...
		assert.deepEqual(
			parentSnapshot?.stages.map((stage) => stage.name),
			[],
		);
		assert.equal(
			store.runs().some((runSnapshot) => runSnapshot.name === "exit-delayed-child"),
			false,
		);
	});
	...
	test("skips a workflow boundary without launching the child when exit is selected before launch", async () => {
		...
		assert.equal(
			store.runs().some((runSnapshot) => runSnapshot.name === "exit-boundary-child"),
			false,
		);
	});
	...
	test("parent exit while a child workflow is in flight lets the child finalize cleanup", async () => {
						...
						create: async () =>
							({
								...
								async dispose() {
									...
									entries.push({
										type: "test.child-session.dispose",
		...
		assert.equal(
			childSnapshot.stages.some((stage) => stage.attachable === true),
			false,
		);
		...
		assert.equal(
			store
				.runs()
				.some((runSnapshot) =>
					runSnapshot.stages.some(
						(stage) => stage.status === "running" || stage.status === "pending" || stage.attachable === true,
					),
				),
			false,
		);

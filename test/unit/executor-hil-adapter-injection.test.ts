...
import {
	...
	Type,
	test,
	type WorkflowCustomUiFactory,
	type WorkflowCustomUiOptions,
	type WorkflowUIAdapter,
	workflow,
...
describe("executor.run — HIL adapter injection", () => {
	...
	test("ctx.ui.custom delegates to an injected adapter when prompt nodes are disabled", async () => {
		...
		const uiAdapter = {
			...
			custom: async <T>(
				_factory: Parameters<
					NonNullable<import("../../packages/workflows/src/shared/types.js").WorkflowUIAdapter["custom"]>
				>[0],
				options?: Parameters<
					NonNullable<import("../../packages/workflows/src/shared/types.js").WorkflowUIAdapter["custom"]>
				>[1],
	...
	test("method-syntax UI adapters preserve this for every ctx.ui method", async () => {
		const uiAdapter = {
			...
			async select<T extends string>(this: { readonly prefix: string }, message: string, options: readonly T[]) {
		...
		const def = workflow({
			...
			run: async (ctx) => {
				const values = {
					...
					custom: await ctx.ui.custom<string>(() => ({ render: () => ["custom"], invalidate: () => undefined }), {
						label: "widget",
					}),
				};
	...
	test("class-instance UI adapters preserve this for every ctx.ui method", async () => {
		class StatefulUiAdapter implements WorkflowUIAdapter {
			...
			async custom<T>(_factory: WorkflowCustomUiFactory<T>, options?: WorkflowCustomUiOptions): Promise<T> {
				return {
					prefix: this.prefix,
					label: options?.label,
				} as T;
	...
	test("primitive-only UI adapters reject ctx.ui.custom with the unavailable UI message", async () => {
		...
		const def = workflow({
			name: "hil-custom-adapter-missing-wf",
			description: "",
			inputs: {},
			outputs: {},
			run: async (ctx) => {

...
describe("executor.run — lifecycle persistence", () => {
	...
	test("appends ordered run.start → stage.start → stage.end → run.end on success", async () => {
		...
		assert.deepEqual(types, ["workflow.run.start", "workflow.stage.start", "workflow.stage.end", "workflow.run.end"]);
		assert.equal(
			calls.every((call) => !call.type.includes("durable")),
			true,
		);
	});
	...
	test("non-fail-fast parallel rate limits persist run.blocked without ending the run", async () => {
		...
		const def = workflow({
			name: "parallel-blocked-persist-wf",
			description: "",
			inputs: {},
			outputs: {},
			run: async (ctx) => {
	...
	test("fail-fast skipped queued parallel stages persist start before end", async () => {
		...
		const def = workflow({
			name: "fail-fast-pending-persist-wf",
			description: "",
			inputs: {},
			outputs: {},
			run: async (ctx) => {

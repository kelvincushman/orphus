...
function capturedStageWorkflow(name: string, stageReady: PromiseWithResolvers<StageContext>): WorkflowDefinition {
	return workflow({
		name,
		description: "",
		inputs: {},
...
async function runStageContextDelivery(input: {
	...
	const runPromise = run(
		capturedStageWorkflow(input.workflowName, stageReady),
		{},
		{
			adapters: {
				agentSession: {
...
describe("executor — queued follow-up continuation delivery", () => {
	...
	test("ctx.sendUserMessage during streaming queues a follow-up and injects with the gate disabled", async () => {
		...
		assert.deepEqual(recorder.sendUserMessageCalls, [{ text: "workflow-author user message", behavior: "followUp" }]);
	});
	...
	test("the injected continuation prompt does not re-arm itself", async () => {
		...
		const result = await runPromise;
		assert.equal(result.status, "completed");
		assert.equal(recorder.promptCalls.filter((text) => text === RESUME_CONTINUATION_PROMPT).length, 1);
	});

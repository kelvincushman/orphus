...
describe("executor.run", () => {
		...
		const def = workflow({
			...
			run: async (ctx) => {
				const results = await ctx.chain(
					[{ name: "scout" }, { name: "planner" }, { name: "worker", task: "implement from {previous}" }],
					{ task: "analyze auth" },
				);
	...
	test("ctx.parallel follows direct workflow shared task fallback", async () => {
		...
		const def = workflow({
			...
			run: async (ctx) => {
				const results = await ctx.parallel([{ name: "frontend", task: "audit UI" }, { name: "backend" }]);
	...
	test("ctx.stage schema opt-in registers structured_output and returns captured params", async () => {
		const calls: CreateAgentSessionOptions[] = [];
		const DecisionSchema = Type.Object({ approved: Type.Boolean() }, { additionalProperties: false });
		const def = workflow({
		...
		const def = workflow({
			...
			run: async (ctx) => {
				...
				return {
					...
					plainStructured:
						chainResults[1]?.structured !== undefined || parallelResults[1]?.structured !== undefined,
				};
			},
		});
		const wfResult = await run(

...
describe("executor.run", () => {
	test("continuation replays workflow boundary with serializable raw output", async () => {
		...
		const sourceBoundary = source.stages.find((stage) => stage.name === "workflow:resume-uncloneable-raw-child")!;
		...
		const boundary = continued.stages.find((stage) => stage.name === "workflow:resume-uncloneable-raw-child")!;
	...
	test("ctx.workflow rejects non-workflow definitions before starting a child run", async () => {
		const parent = workflow({
			name: "direct-definition-validation-parent",
			description: "",
			inputs: {},
			outputs: {},
			run: async (ctx) => {
		...
		assert.match(result.error ?? "", /ctx\.workflow\(definition\) requires a workflow definition/);
	...
	test("continuation replays repeated concurrent ctx.workflow boundaries for the same alias", async () => {
		...
		const sourceBoundaries = source.stages.filter((stage) => stage.name === "workflow:resume-import-repeated-child");
	...
	test("continuation maps legacy ctx.workflow boundary and reruns child when replay metadata is absent", async () => {
		...
		assert.equal(firstRun.status, "failed");
		const source = st.runs().find((candidate) => candidate.id === firstRun.runId)!;
		const sourceBoundary = source.stages.find((stage) => stage.name === "workflow:resume-import-legacy-child")!;
		...
		const boundary = continued.stages.find((stage) => stage.name === "workflow:resume-import-legacy-child")!;

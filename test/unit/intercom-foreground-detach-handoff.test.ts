...
import {
	...
	createHeavyProxy,
} from "../../packages/intercom/lazy-heavy-proxy.js";
import type { Message, SessionInfo } from "../../packages/intercom/types.js";
import { sleep } from "../helpers/runtime.js";

const from = (id: string, name: string): SessionInfo => ({
	id,
	name,
	cwd: "/tmp",
	model: "m",
...
const message = (id: string, expectsReply: boolean): Message => ({
	id,
	timestamp: 1,
	expectsReply,
	content: { text: "hello" },
...
function fixture() {
	...
	const events = {
		emit(channel: string, payload: object) {
			emitter.emit(channel, payload);
		},
		on(channel: string, handler: (payload: object) => void) {
...
describe("broker foreground delivery handshake", () => {
	test("probes and commits the exact owner before surfacing a blocking detach", async () => {
		...
		emitter.on(
			INTERCOM_DETACH_REQUEST_EVENT,
			(request: { phase?: string; requestId: string; childIntercomTarget?: string; runtimeGeneration?: number }) => {
		...
		const ok = await handoff.deliver({
			from: from("a", "child-a"),
			message: message("q", true),
			generation: 3,
			surface: () => order.push("surface"),
			isCurrent: () => true,
		});
	...
	test("an unmatched sender falls back without surfacing", async () => {
		...
		assert.equal(
			await handoff.deliver({
				from: from("background", "background-child"),
				message: message("q", true),
				generation: 1,
				surface: () => surfaced++,
				isCurrent: () => true,
	...
	test("reset immediately abandons and cleans up an in-flight probe", async () => {
		...
		const delivery = handleForegroundInboundDelivery({
			handoff,
			from: from("a", "child-a"),
			message: message("reset-probe", true),
			generation: 1,
			surface: () => surfaced++,
			isCurrent: () => true,
	...
	test("reset immediately abandons and cleans up an in-flight commit", async () => {
		...
		const delivery = handleForegroundInboundDelivery({
			handoff,
			from: from("a", "child-a"),
			message: message("reset-commit", true),
			generation: 1,
			surface: () => surfaced++,
			isCurrent: () => true,
			onUnclaimed: () => unclaimed++,

...
function intercomMessage(id: string, group?: string, channel?: "supervisor") {
	...
	return {
		...
		content: id,
		display: true,
		details: { from, message, bodyText: id, channel },
	} as const;
}
...
test("fallback batch commits successful members and retries only the failed suffix", async () => {
	...
	const pi = {
		events: {
			on(_name: string, next: typeof handler) {
				handler = next;
				return () => {};
			},
		},
		async sendMessage(message: ReturnType<typeof intercomMessage>) {
			if (message.content === "second" && rejectSecond) {
				rejectSecond = false;
	...
	const route = () => {
		const payload = { handled: false, batch: true, messages, options: { triggerTurn: true } } as {
			handled: boolean;
			batch: boolean;
...
test("late relay suppresses isolated peer chatter to a differently grouped parent", async () => {
	...
	const payload = {
		...
		handled: boolean;
		batch: boolean;
		messages: ReturnType<typeof intercomMessage>[];
		options: object;
		completion?: Promise<void>;
	};

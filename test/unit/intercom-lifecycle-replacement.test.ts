...
function fixture(disconnectRejects = false) {
	...
	const deps = {
		...
		setRuntimeStarted(value: boolean) {
			live = value;
		},
		incrementRuntimeGeneration: () => ++generation,
		...
		setAgentRunning() {},
		activeTools,
		getLiveContext: (_ctx?: object, expected = generation) =>
			live && expected === generation ? ({} as never) : null,
		scheduleReconnect() {},
		rejectReplyWaiter(error: Error) {
			rejected.push(error.message);
		},
		replyTracker: { reset() {}, endTurn() {}, beginTurn() {} },
		pendingIdleMessages: pending,
		clearInboundFlushTimer() {},
		scheduleInboundFlush(delayMs?: number) {
			inboundFlushDelays.push(delayMs);
		},
		syncPresenceStatus() {},
	...
	const connect = async () => {
		const next = {
			...
			async disconnect() {
				this.disconnects++;
				if (disconnectRejects) throw new Error("disconnect failed");
			},
		};
		clients.push(next);
		client = next;
	...
	return {
		...
		get sessionEnvRestores() {
			return sessionEnvRestores;
		},
	};
}

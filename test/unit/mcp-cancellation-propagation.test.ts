...
test("one direct caller aborts promptly while a survivor waits for shared readiness", async () => {
	...
	const readiness = new Promise<McpExtensionState>((resolve) => {
		release = resolve;
	});
	let ensureCalls = 0;
	const execute = createDirectToolExecutor(
	...
	void cancelled.then(
		...
		(error) => {
			cancellationOutcome = error === reason;
		},
	);
	controller.abort(reason);
	await waitForCondition(
		"cancelled caller to settle before shared readiness",
...
test("a direct caller abort observes a later shared readiness rejection", async () => {
	...
	const readiness = new Promise<McpExtensionState>((_resolve, reject) => {
	...
	try {
		...
		controller.abort(reason);
		await waitForCondition(
			"cancelled caller to settle before shared readiness rejects",
			() => cancellationOutcome !== undefined,
		);
		...
		setImmediate(() => {
			rejectionCheckpointReached = true;
		});
...
test("pre-aborted direct calls do not start shared readiness", async () => {
	...
	await assert.rejects(execute("call", {}, controller.signal, undefined, {} as never), (error) => error === reason);

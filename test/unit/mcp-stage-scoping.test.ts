...
import { type AgentSessionAdapter, makeMockSession } from "./stage-runner-helpers.js";

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

type OrderEvent = "stageStart" | "mcpSet" | "adapterCall" | "prompt" | "sendUserMessage" | "mcpClear" | "stageEnd";
...
describe("MCP stage scoping — call order", () => {
	...
	test("sendUserMessage follow-on turns reapply and clear the stage MCP scope", async () => {
		...
		assert.deepEqual(
			order.filter((event) => event !== "stageStart" && event !== "stageEnd"),
			["mcpSet", "prompt", "mcpClear", "mcpSet", "sendUserMessage", "mcpClear"],
		);
	});
	...
	test("streaming sendUserMessage does not clear the active prompt MCP scope", async () => {
		...
		assert.deepEqual(
			order.filter((event) => event !== "stageStart" && event !== "stageEnd"),
			["mcpSet", "prompt", "sendUserMessage", "mcpClear"],
		);
	});
	...
	test("concurrent stages: each gets distinct stageId in setScope", async () => {
		...
		const mcpPort: WorkflowMcpPort = {
			setScope(stageId, allow) {
				setCalls.push({ stageId, allow });
			},
			clearScope(stageId) {
				clearCalls.push(stageId);
			},
		};

...
async function createSdkPair(handler: (context: ToolHandlerContext) => Promise<CallToolResult>): Promise<SdkPair> {
	const server = new Server({ name: "timeout-test-server", version: "1.0.0" }, { capabilities: { tools: {} } });
...
function createConnectedState(client: Client, timeoutMs?: number): { state: McpExtensionState; getInFlight(): number } {
	const definition = {
		command: "bun",
...
test("timeoutMs config rejects non-numeric, non-positive, and NaN values clearly", () => {
	...
	assert.equal(api.MCP_TIMEOUT_MS_CONFIG_ERROR, "timeoutMs must be a finite number greater than 0 milliseconds");
	...
	for (const { value, rendered } of invalidValues) {
		assert.throws(
			...
			(error) =>
				...
				error.message ===
					'Invalid MCP configuration for server "demo": ' +
...
test("UI server timeout response names the server and configured inactivity limit", async () => {
	...
	try {
		...
		assert.equal(payload.error, 'MCP tool call to server "server" timed out after 30 ms of inactivity.');
	} finally {
		handle.close("test complete");
	}
});

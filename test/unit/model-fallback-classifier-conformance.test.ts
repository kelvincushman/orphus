...
const CONFORMANCE_FIXTURES: readonly Fixture[] = [
	...
	{
		label: "status 400",
		failure: { status: 400, message: "bad request" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "status 413",
		failure: { statusCode: 413, message: "payload too large" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "status 422",
		failure: { httpStatus: 422, message: "unprocessable" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		...
		kind: "auth_on_candidate_provider",
		retryable: true,
	},
	{
		label: "status 403",
		failure: { status: 403, message: "forbidden" },
		kind: "auth_on_candidate_provider",
		retryable: true,
	},
	{
		label: "status 404",
		failure: { status: 404, message: "model missing" },
		kind: "model_unavailable",
		retryable: true,
	},
	{
		label: "status 408",
		failure: { status: 408, message: "request timeout" },
	...
	{
		label: "status 503",
		failure: { status: 503, message: "service unavailable" },
		kind: "provider_unavailable",
		retryable: true,
	},
	// Request-incompatible provider codes (numeric, string, and named).
	{
		label: "code 413 numeric",
		failure: { code: 413, message: "too big" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "code 422 string",
		failure: { code: "422", message: "unprocessable" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "code invalid_request_error",
		failure: { code: "invalid_request_error", message: "localized" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "code bad_request",
		failure: { code: "bad_request", message: "localized" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "code context_length_exceeded",
		failure: { code: "context_length_exceeded", message: "localized" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "code request_too_large",
		failure: { code: "request_too_large", message: "localized" },
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "code max_tokens",
		failure: { code: "max_tokens", message: "localized" },
		kind: "request_incompatible",
		retryable: true,
	},
	// Request-incompatible messages.
	{
		...
		failure: new Error("This model's context length exceeded"),
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "message context window",
		failure: new Error("context window exceeded for candidate"),
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "message request too large",
		failure: new Error("request too large for this model"),
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "message unsupported tool",
		failure: new Error("unsupported tool: computer-use"),
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "message parameter not supported",
		failure: new Error("parameter not supported by this model"),
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "message invalid request",
		failure: new Error("invalid request"),
		kind: "request_incompatible",
		retryable: true,
	},
	{
		label: "message bad request",
		failure: { message: "400 bad request" },
		kind: "request_incompatible",
		retryable: true,
	},
	...
	{
		label: "benign message containing too large",
		failure: new Error("generated diff is too large to display inline"),
		kind: "request_incompatible",
		retryable: true,
	},
	// Refusals/cancellations must win over request-incompatible wrappers.
	{
		label: "aborted stopReason wrapper",
		failure: { status: 400, stopReason: "aborted", errorMessage: "aborted" },
		kind: "cancelled",
		retryable: false,
	},
	{
		label: "AbortError name wrapper",
		failure: { statusCode: 422, name: "AbortError", message: "aborted by user" },
		kind: "cancelled",
		retryable: false,
	},
	{
		label: "content filter wrapper",
		failure: { httpStatus: 400, message: "content_filter" },
		kind: "task_failure",
		retryable: false,
	},
	{
	...
	{ label: "abort cause under invalid request", failure: abortWrappedError(), kind: "cancelled", retryable: false },
	{
		...
		failure: { message: "400 bad request", cause: { message: "request was cancelled" } },
		kind: "cancelled",
		retryable: false,
	},
	{
		label: "task failure cause under bad request",
		failure: { errorMessage: "bad request", cause: { message: "command failed: exit 1" } },
		kind: "task_failure",
		retryable: false,
	},
	{
		label: "content filter diagnostic under invalid_request_error",
		failure: { message: "invalid_request_error", diagnostics: [{ error: { finish_reason: "content_filter" } }] },
		kind: "task_failure",
		retryable: false,
	},
	{
		label: "safety diagnostic under 400 wrapper",
		failure: { errorMessage: "400 bad request", diagnostics: [{ error: { message: "blocked by safety policy" } }] },
		kind: "task_failure",
		retryable: false,
	},
	{
		label: "task failure diagnostic under 422",
		failure: { status: 422, diagnostics: [{ error: { message: "command failed" } }] },
		kind: "task_failure",
		retryable: false,
	},
	// Retryable nested causes must not flip a retryable wrapper.
	{
		label: "rate limit cause under bad request",
		failure: { errorMessage: "bad request", cause: { message: "rate limit exceeded" } },
		kind: "request_incompatible",
		retryable: true,
	},
	// Common retryable classifications shared by both copies.
	{
		...
		failure: new Error("Rate limit exceeded, retry later"),
		kind: "rate_limit",
		retryable: true,
	},
	{
		label: "message api key",
		failure: new Error("No API key found for provider"),
		kind: "auth_on_candidate_provider",
		retryable: true,
	},
	{
		label: "message model not found",
		failure: new Error("model not found: foo/bar"),
		kind: "model_unavailable",
		retryable: true,
	},
	{
		label: "code etimedout",
		failure: { code: "ETIMEDOUT", message: "socket timed out" },
		kind: "network_timeout",
		retryable: true,
	},
	{
		label: "message overloaded",
		failure: new Error("provider is overloaded (529)"),
		kind: "provider_unavailable",
		retryable: true,
	},
	...
	{
		label: "message usage limit reached",
		failure: new Error("Codex error: The usage limit has been reached"),
		kind: "rate_limit",
		retryable: true,
	},
	{
		label: "session usage limit errorMessage",
		failure: { stopReason: "error", errorMessage: "Codex error: The usage limit has been reached" },
		kind: "rate_limit",
		retryable: true,
	},
	{
		label: "usage limit cause under stage wrapper",
		failure: { message: "stage session failed", cause: { message: "Codex error: The usage limit has been reached" } },
		kind: "rate_limit",
		retryable: true,
	},
	{
		label: "code usage_limit_reached",
		failure: { code: "usage_limit_reached", message: "localized" },
		kind: "rate_limit",
		retryable: true,
	},
	{
		label: "code insufficient_quota",
		failure: { code: "insufficient_quota", message: "localized" },
		kind: "rate_limit",
		retryable: true,
	},
	// Cancellation nested under a usage-limit wrapper must stay non-retryable.
	{
		label: "abort cause under usage limit wrapper",
		failure: {
	...
	{
		label: "message usage_limit_reached token",
		failure: new Error("usage_limit_reached: retry later"),
		kind: "rate_limit",
		retryable: true,
	},
	{
		label: "message usage-limit hyphen",
		failure: new Error("provider usage-limit exceeded"),
		kind: "rate_limit",
		retryable: true,
	},
	{
		label: "message USAGE_LIMIT_EXCEEDED upper",
		failure: new Error("USAGE_LIMIT_EXCEEDED"),
		kind: "rate_limit",
		retryable: true,
	},
	{
		label: "message usagelimit joined",
		failure: new Error("usagelimit reached"),
		kind: "rate_limit",
		retryable: true,
	},
	...
	{
		label: "message unrelated index limit not usage",
		failure: new Error("array index limit reached"),
		kind: "unknown",
		retryable: false,
	},
	// Non-retryable terminal failures.
	{
		label: "message tests failed",
		failure: new Error("tests failed: 3 failures"),
		kind: "task_failure",
		retryable: false,
	},
	{ label: "message interrupted", failure: new Error("interrupted by user"), kind: "cancelled", retryable: false },
	{
		label: "unknown structured failure",
		failure: { message: "something inexplicable happened" },
		kind: "unknown",
		retryable: false,
	},
	{
...
describe("model fallback classifier conformance (subagents vs workflows)", () => {
	...
	test("both classifiers agree on the shared failure corpus", () => {
		for (const fixture of CONFORMANCE_FIXTURES) {
			...
			assert.equal(
				subagentsIsRetryable(fixture.failure),
				fixture.retryable,
				`subagents retryable for ${fixture.label}`,
			);
			assert.equal(
				workflowsIsRetryable(fixture.failure),
				fixture.retryable,
				`workflows retryable for ${fixture.label}`,
			);
		}
	});

...
describe("main-chat retry classifies usage-limit exhaustion as retryable", () => {
	...
	test("usage-limit and quota error messages are retryable so fallbackModels can advance", () => {
		...
		assert.equal(
			_isRetryableError.call(
				session as never,
				errorMessage({ errorMessage: "quota exceeded for this billing period" }),
			),
			true,
		);
	});
	...
	test("usage-limit tokens with non-space separators are retryable in main chat too (three-path parity)", () => {
		for (const message of [
			...
			assert.equal(_isRetryableError.call(session as never, errorMessage({ errorMessage: message })), true, message);
		}
	});
	...
	test("aborted turns and non-error stops are not retryable", () => {
		...
		assert.equal(_isRetryableError.call(session as never, errorMessage({ errorMessage: "Tool not found" })), false);
	});
});

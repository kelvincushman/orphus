...
describe("prompt callsite stack frame filtering", () => {
	test("filters source workflow runtime frames", () => {
		...
		const frame = normalizedPromptCallsiteFrame(`    at promptReplayKey (${runtimePath}:262:13)`);
	...
	test("preserves workflow author frames", () => {
		...
		assert.equal(
			normalizedPromptCallsiteFrame(
				"    at workflow (packages/coding-agent/dist/builtin/workflows/builtin/tournament.ts:618:12)",
			),

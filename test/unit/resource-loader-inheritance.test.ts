...
describe("DefaultResourceLoader inheritance snapshots", () => {
	test("ExtensionAPI exposes inherited resource options and child loaders consume them without sharing the parent loader", async () => {
		...
		writeFileSync(join(externalPackage, "index.ts"), "export default function inheritedExtension() {}\n");
		...
		const parentLoader = new DefaultResourceLoader({
			...
			extensionFactories: [
				(pi) => {
					observedSnapshots.push(pi.getResourceLoaderInheritanceSnapshot?.() ?? {});
				},
			],
		});
		...
		assert.deepEqual(observedSnapshots[0]?.additionalExtensionPaths, [externalPackage]);
		assert.deepEqual(observedSnapshots[0]?.additionalSkillPaths, [skillFile]);
		assert.deepEqual(observedSnapshots[0]?.builtinPackagePaths, ["/repo/packages/workflows"]);
		assert.deepEqual(observedSnapshots[0]?.appendSystemPrompt, ["Parent append prompt"]);
		...
		assert.deepEqual(childSnapshot.trustedBorrowedProjectLocalSources, [externalPackage]);

...
function finding(overrides: Partial<ConsolidatableFinding> & { readonly title?: string } = {}): ConsolidatableFinding {
	return {
...
describe("findingBlocksClosure", () => {
	...
	test("beyond_objective and contradicts_objective never block, preserving literal-contract scope controls", () => {
		for (const alignment of ["beyond_objective", "contradicts_objective"]) {
			assert.equal(findingBlocksClosure({ objective_alignment: alignment, priority: 0 }), false, alignment);
		}
	});
	...
	test("missing or unknown alignment blocks so ambiguity never silently approves", () => {
		...
		assert.equal(findingBlocksClosure({ objective_alignment: "made_up_alignment", priority: 3 }), true);
	});
});
...
describe("unresolvedClosureFindings / closureGapSummary", () => {
	...
	test("summary is inspectable and bounded", () => {
		const many = Array.from({ length: 7 }, (_, index) => finding({ title: `[P1] Gap ${index + 1}`, priority: 1 }));

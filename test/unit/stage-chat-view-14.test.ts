...
describe("StageChatView streaming lifecycle", () => {
	test("terminal and non-running stage states suppress stale live-handle streaming and animation", () => {
		const statuses = ["completed", "failed", "skipped", "blocked", "paused"] as const;
		...
		for (const status of statuses) {
			...
			assert.doesNotMatch(renderText(view), /Working/, `${status} stage should not render a stale working spinner`);
			assert.equal(view._hasAnimationTick, false, `${status} stage should not keep a stale animation tick`);
			view.dispose();
		}
	});

	test("terminal store transitions repaint and clear stale sdk busy animation", () => {
		const terminalStatuses = ["completed", "failed", "skipped", "paused", "blocked"] as const;
	...
	test("terminal handle states suppress stale live-handle streaming and animation", () => {
		for (const alias of terminalOrNonStreamingAliases()) {
			...
			assert.doesNotMatch(renderText(view), /Working/, `${alias} handle should not render a stale working spinner`);
			assert.equal(view._hasAnimationTick, false, `${alias} handle should not keep a stale animation tick`);
			view.dispose();
		}
	});
	...
	test("retained terminal stage chats still show and animate genuine agent work", () => {
		...
		for (const status of statuses) {
			...
			assert.match(renderText(view), /Working/, `${status} retained stage should render genuine agent_start work`);
			assert.equal(view._hasAnimationTick, true, `${status} retained stage should animate genuine agent_start work`);
			view.dispose();
		}
	});

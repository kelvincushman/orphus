...
describe("coerceStageInputAnswer", () => {
	...
	test("questions[] with selected → optionLabels", () => {
		assert.deepEqual(coerceStageInputAnswer({ questions: [{ selected: ["A", "C"] }] }), { optionLabels: ["A", "C"] });
	});
	...
	test("answers[] WITHOUT questionIndex is normalized, not raw-forwarded", () => {
		...
		assert.deepEqual(coerceStageInputAnswer({ answers: [{ selected: ["A", "B"] }] }), { optionLabels: ["A", "B"] });
	});

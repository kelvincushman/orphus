...
import type { AgentSessionAdapter, InternalStageContext, StageSessionCreateOptions } from "./stage-runner-helpers.js";
import {
	assert,
	createStageContext,
	makeMockSession,
...
describe("createStageContext — sendUserMessage", () => {
	...
	test("ignores replayed public starts before a hook-ignoring adapter owns the turn", async () => {
		...
		const publicAdapter = {
			async create() {
				return session as unknown as PublicStageSessionRuntime;
			},
	...
	test("does not retroactively reject delivery authorized before asynchronous handling", async () => {
		...
		const deliveryPromise = sendStageUserMessage(session, "accepted before terminal transition", undefined, () => {
	...
	test("rejects multimodal fallback when the runtime lacks native sendUserMessage", async () => {
		...
		await ctx.__ensureSession();
		await assert.rejects(() => ctx.sendUserMessage(content), /does not support non-string sendUserMessage content/);
	});

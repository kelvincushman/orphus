...
describe("subagent async widget hydration (issue #1146)", () => {
	test("hydrates an active status-visible run and renders the widget belowEditor", () => {
		...
		assert.ok(
			widgetCalls.some((call) => call.options?.placement === "belowEditor"),
	...
	test("keeps a mounted active widget visible across reset and hydration", () => {
		...
		const resultsDir = makeTempRoot("atomic-subagent-widget-results-");
		const state = makeState(cwd, "session-current");
		writeStatus(asyncRoot, "run-visible", makeStatus("run-visible", cwd, { sessionId: "session-current" }));
		...
		const widgetCalls = [...first.widgetCalls, ...reset.widgetCalls, ...hydrate.widgetCalls];
		...
		assert.equal(
			widgetCalls.filter((call) => call.options?.placement === "belowEditor").length,
			1,
			"active hydration after reset should reuse the mounted widget",
		);
		assert.equal(hydrate.renderCount(), 1, "active hydration after reset should render in place once");
	});
	...
	test("unmounts a mounted widget after reset and hydration finds no active jobs", () => {
		...
		tracker.resetJobs(reset.ctx);
		const afterResetWidgetCalls = [...first.widgetCalls, ...reset.widgetCalls, ...hydrate.widgetCalls];
		assert.equal(
		...
		tracker.hydrateActiveJobs(hydrate.ctx);
		const afterHydrateWidgetCalls = [...first.widgetCalls, ...reset.widgetCalls, ...hydrate.widgetCalls];

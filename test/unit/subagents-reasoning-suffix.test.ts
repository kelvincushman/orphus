...
import {
	...
	THINKING_LEVELS,
} from "../../packages/subagents/src/shared/model-info.js";
...
describe("subagent suffix-first reasoning helpers", () => {
	test("splitKnownThinkingSuffix only recognizes canonical levels", () => {
		assert.deepEqual(splitKnownThinkingSuffix("claude-sonnet-4:high"), {
			baseModel: "claude-sonnet-4",
			thinkingSuffix: ":high",
		});
		assert.deepEqual(splitKnownThinkingSuffix("claude-sonnet-4"), {
			baseModel: "claude-sonnet-4",
			thinkingSuffix: "",
		});
		assert.deepEqual(splitKnownThinkingSuffix("provider:model:ultra"), {
			baseModel: "provider:model:ultra",
			thinkingSuffix: "",
		});
		assert.deepEqual(splitKnownThinkingSuffix("provider:with-colon/model:off"), {
			baseModel: "provider:with-colon/model",
			thinkingSuffix: ":off",
		});
	...
	test("provider-qualified config models with ':' separators survive frontmatter parsing and resolution", () => {
		...
		assert.deepEqual(
			...
			["provider:with-colon/model:off", "openai/gpt-7-preview:high", "anthropic/claude-sonnet-4:medium"],
		);
		// A fully-qualified id absent from the catalog resolves to itself.
		assert.equal(
			resolveModelCandidate("openai/gpt-7-preview:high", models, "anthropic"),
			"openai/gpt-7-preview:high",
		);
		assert.deepEqual(splitKnownThinkingSuffix(frontmatter.model), {
			baseModel: "provider:with-colon/model",
			thinkingSuffix: ":off",
		});
	});
	...
	test("fallbackThinkingLevels applies positionally only when fallback has no suffix", () => {
		assert.deepEqual(
			buildModelCandidates("claude-sonnet-4", ["gpt-5", "claude-sonnet-4:low"], models, "anthropic", undefined, [
				"medium",
				"xhigh",
			]),
...
describe("subagent retry metadata reasoning seams", () => {
	test("foreground retry candidates resolve per-attempt model and reasoning with suffix precedence", () => {
		...
		const candidates = buildModelCandidates("claude-sonnet-4:high", ["gpt-5:low"], models, "anthropic");
		...
		assert.deepEqual(candidates, ["anthropic/claude-sonnet-4:high", "openai/gpt-5:low"]);
	...
	test("legacy no-suffix retry candidates keep legacy thinking as the effective level", () => {
		const candidates = buildModelCandidates("claude-sonnet-4", ["gpt-5"], models, "anthropic");

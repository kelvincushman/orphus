...
import { type AsyncJobState, firstSpinnerChar, theme, withMockedNow } from "./subagents-render-stability-helpers.js";
...
describe("subagent running pulse animation (issue #1084)", () => {
	...
	test("async widget running glyphs reuse foreground pulse frames and ignore wall-clock time", () => {
		...
		const first = withMockedNow(10_000, () => buildWidgetLines([job], theme, 120, false, 10_000, 1).join("\n"));
		const sameFrameLater = withMockedNow(10_000 + RUNNING_ANIMATION_MS, () =>
			buildWidgetLines([job], theme, 120, false, 10_000, 1).join("\n"),
		);
		assert.equal(sameFrameLater, first, "async pulse must not advance from wall-clock time alone");
	...
	test("async widget honours captured now for job, step, and nested running pulse glyphs", () => {
		...
		const sameFrameLater = buildWidgetLines([job], theme, 120, true, 10_000 + RUNNING_ANIMATION_MS, 1).join("\n");
		assert.equal(
			sameFrameLater,
			first,
		...
		assert.notEqual(second, first, "progress/status-update pulse frames should advance job, step, and nested glyphs");
		...
		assert.equal(
			firstSpinnerChar(first),
			undefined,
			"job, step, and nested running glyphs must not render spinner-style glyphs",
		);
		assert.equal(
			firstSpinnerChar(second),
			undefined,
			"job, step, and nested running glyphs must not render spinner-style glyphs",
		);

		const stableA = withMockedNow(20_000, () => buildWidgetLines([job], theme, 120, true, 10_000, 1).join("\n"));
		const stableB = withMockedNow(30_000, () => buildWidgetLines([job], theme, 120, true, 10_000, 1).join("\n"));
		assert.equal(
			stableB,
			stableA,
	...
	test("multi-job async widget list honours captured now for header and row pulse glyphs", () => {
		...
		const sameFrameLater = buildWidgetLines(jobs, theme, 120, false, 10_000 + RUNNING_ANIMATION_MS, 1).join("\n");
		assert.equal(
			sameFrameLater,
			first,

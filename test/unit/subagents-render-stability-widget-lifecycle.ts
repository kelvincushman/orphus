...
describe("async widget animation ticker lifecycle", () => {
	...
	test("mounted async widget advances pulse only when the job snapshot changes", () => {
		...
		const stableB = withMockedNow(40_000, () => component.render(120).join("\n"));
		assert.equal(stableB, stableA, "same job snapshot must not advance the async pulse frame");

		withMockedNow(50_000, () => renderWidget(ctx, [{ ...runningJob(), toolCount: 2, updatedAt: 11_000 }]));
		const advanced = component.render(120).join("\n");
		assert.notEqual(advanced, stableA, "semantic job snapshot updates should advance the async pulse frame");
	});
	...
	test("mounted async widget keeps last expanded state across stale stage-exit context re-renders", () => {
		...
		assert.match(expanded, /bash: bun test/, "expanded widget should show live detail from the current UI state");
	...
	test("visible async widget remounts when the logical owner changes", () => {
		...
		assert.equal(first.widgetCalls.length, 2, "stale context should mount once and then be cleared on owner switch");
		assert.equal(
		...
		assert.equal(first.renders(), 0, "owner switch should not request an in-place render on the stale context");
		assert.equal(second.renders(), 0, "owner switch should mount rather than request render before mounting");
	...
	test("running jobs do not drive periodic re-renders without status updates", async () => {
		...
		assert.equal(renders(), 0, "async widget pulse must not schedule cosmetic timer re-renders");

		renderWidget(ctx, [{ ...runningJob(), toolCount: 2, updatedAt: 11_000 }]);
		assert.equal(renders(), 1, "semantic status updates still request an in-place widget render");

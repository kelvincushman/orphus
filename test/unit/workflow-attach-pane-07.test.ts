...
describe("WorkflowAttachPane", () => {
	...
	test("Ctrl+C prompt skip does not consume the next graph Enter", async () => {
		...
		assertNextGraphEnterAttaches(pane, "stage-b", "first graph-mode Enter after Ctrl+C attaches");
		pane.dispose();
	});
	...
	for (const [key, expected] of [
		...
		test(`confirm ${key} prompt answer does not consume the next graph Enter`, async () => {
			...
			assertNextGraphEnterAttaches(pane, "stage-b", `first graph-mode Enter after ${key} attaches`);
			pane.dispose();
		});
	}
});

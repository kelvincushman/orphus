...
describe("classifyWorkflowFailure", () => {
	...
	test("classifies non-contiguous invalid API key fallback messages as invalid credentials", () => {
		for (const message of ["The API key provided is invalid", "The API key you supplied is incorrect"] as const) {
	...
	test("uses structured codes and causes before message fallback", () => {
		...
		const rateLimit = classifyWorkflowFailure(
	...
	test("classifies AggregateError inner provider failures before wrapper text", () => {
		const rateLimited = classifyWorkflowFailure(
			new AggregateError(
		...
		const invalidKey = classifyWorkflowFailure(
			new AggregateError([{ status: 401, message: "Unauthorized" }], "atomic-workflows: 1 parallel step failed"),
		);
		...
		const failure = classifyWorkflowFailure(
			new Error("wrapper", {
				cause: new AggregateError(
					[new Error("domain validation failed"), { status: 429, message: "too many requests" }],
